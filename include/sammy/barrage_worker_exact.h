...
class CliqueSatDSaturExactSolverCore {
  public:
    static std::unique_ptr<CliqueSatDSaturExactSolverCore> factory(
        PortfolioSolver* solver, PortfolioElementWithCore<CliqueSatDSaturExactSolverCore>* our_element)
    {
        return std::make_unique<CliqueSatDSaturExactSolverCore>(solver, our_element->get_mutable_recorder());
    }

    using IncrementalSolver = CadicalSolver;
    using CSDSSolver = CliqueSatDSaturSolver<IncrementalSolver>;

    explicit CliqueSatDSaturExactSolverCore(PortfolioSolver* solver, EventRecorder* local_recorder) :
        m_solver(solver),
        ...
        )
    {
        ...
        m_exact.set_lower_bound_callback([this] (std::size_t lb, const std::vector<Vertex>& subgraph) {
            if(lb == subgraph.size()) {
                m_solver->report_mes(subgraph, "C&P/SATDSatur Exact Solver");
            } else {
                m_solver->report_lower_bound(lb, subgraph, "C&P/SATDSatur Exact Solver");
            }
        });
  ...
  private:
    const ImpliedVertexCache* p_check_cache() {
        ...
        if(!icache.have_reduced_universe()) {
            throw std::logic_error("Did not reduce universe in exact portfolio element!");
        }
        return &icache;
    }
