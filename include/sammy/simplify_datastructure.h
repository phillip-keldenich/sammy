...
class SimplifyDatastructure {
  public:
        ...
        m_var_presence(formula.num_vars())
    {
        ...
        for(CRef c = 1, n = formula.literal_db_size(); c < n; c = formula.next_clause(c)) {
     ...
     * of literals is a (not necessarily strict) superset of any clause in the formula.
     * @param set A set (must have .count(element) method) of literals.
     * @return true iff a clause of the formula subsumes the given set of literals.
     */
    template<typename SetType>
    bool has_clause_subsuming(const SetType& set)
    {
        auto is_contained = [&] (Lit l) -> bool {
            return set.count(l);
        };
    ...
    bool has_duplicate_binary_clause() {
        ...
        for(const SCVec& cl : m_clauses) {
            ...
            std::pair<Lit,Lit> edge{std::min(cl[0], cl[1]), std::max(cl[0], cl[1])};
    ...
    void mark_eliminated(Var v) noexcept {
    ...
    void remove_eliminated_clauses() {
        ...
        m_clauses.erase(std::remove_if(m_clauses.begin(), m_clauses.end(), clause_eliminated),
                        m_clauses.end());
    ...
    bool apply_fixes_and_equalities(const std::vector<Lit>& old_to_new) {
        ...
        for(Var v = 0, nv = old_to_new.size() / 2; v < nv; ++v) {
            if(m_variable_eliminated[v]) continue;
            Lit p = lit::positive_lit(v);
            Lit m = old_to_new[p];
            if(m == simplify::fixed_positive() || m == simplify::fixed_negative()) {
                p_apply_fix(v, m);
                result = true;
            } else if(m != p) {
                p_apply_remap(v, m);
    ...
    void add_clauses(const ClauseDB& clause_db, ClauseCounts old_clause_counts) {
        auto new_u = clause_db.unary_literals(old_clause_counts.unary_clause_end,
                                              clause_db.num_unaries());
        auto new_b = clause_db.binary_clauses(old_clause_counts.binary_clause_end,
                                              clause_db.num_binaries());
        for(Lit u : new_u) {
            m_clauses.emplace_back(1, u);
        }
        ...
        for(CRef c = old_clause_counts.long_clause_end,
                 n = clause_db.literal_db_size(); c < n; c = clause_db.next_clause(c)) 
        {
    ...
    void sort_clauses() {
        auto sort_clause = [] (SCVec& clause) { std::sort(clause.begin(), clause.end()); };
        auto cmp_clauses = [] (const SCVec& c1, const SCVec& c2) -> bool {
            return c1.size() < c2.size() || (c1.size() == c2.size() && 
                       std::lexicographical_compare(c1.begin(), c1.end(), c2.begin(), c2.end()));
        };
    ...
    std::vector<bool> reconstruct_solution(const SimplifiedInstance& simp,
                                           const std::vector<bool>& simp_sol) const
    {
        ...
        IteratorRange stack_order{m_reconstruction_stack.rbegin(), m_reconstruction_stack.rend()};
    ...
    std::vector<Vertex> reconstruct_lb(const SimplifiedInstance& simp,
                                       const std::vector<Vertex>& simp_lb) const
    {
        ...
        auto transform_literal = [&] (Lit l) {
            ...
            return lit::negative(l) ? lit::negative_lit(ov) : lit::positive_lit(ov);
        };
        auto transform_vertex = [&] (Vertex simp_v) {
            return Vertex{transform_literal(simp_v.first), transform_literal(simp_v.second)};
        };
        std::transform(simp_lb.begin(), simp_lb.end(), std::back_inserter(result), transform_vertex);
        return result;
    }
                       ...
                       const std::vector<std::vector<bool>>& sample) const
    {
        ...
        std::transform(sample.begin(), sample.end(),
                       std::back_inserter(result), reconstruct);
        return result;
    }
    ...
    std::vector<SCVec>& clauses() noexcept {
        return m_clauses;
    }

    const std::vector<SCVec>& clauses() const noexcept {
        return m_clauses;
    }

    void replace_all_binaries(const std::vector<std::pair<Lit,Lit>>& binaries) {
        ...
        auto cnt_long = std::count_if(m_clauses.begin(), m_clauses.end(), is_long);
        result.reserve(binaries.size() + cnt_long);
        std::transform(binaries.begin(), binaries.end(), std::back_inserter(result), pair_to_scvec);
        std::copy_if(m_clauses.begin(), m_clauses.end(), std::back_inserter(result), is_long);
        m_clauses.swap(result);
    }

  private:
    ClauseDB p_compress_clauses(Var new_count, const std::vector<Lit>& old_to_new) {
        ...
        for(const SCVec& c : m_clauses) {
            ...
            assert(std::find(buffer.begin(), buffer.end(), simplify::eliminated()) == buffer.end());
            result.add_clause(buffer.data(), buffer.data() + buffer.size());
        }
        return result;
    }

    void p_fix_and_eq_rewrite_clause(SCVec& c, const std::vector<Lit>& old_to_new) {
        auto transform_literal = [&] (Lit l) { return old_to_new[l]; };
        auto not_fixed_negative = [&] (Lit l) { return old_to_new[l] != simplify::fixed_negative(); };
        auto fixed_positive = [&] (Lit l) { return old_to_new[l] == simplify::fixed_positive(); };
        if(std::any_of(c.begin(), c.end(), fixed_positive)) {
            c.clear();
            return;
        }
        c.erase(copy_transformed_if(c.begin(), c.end(), c.begin(),
                                    transform_literal, not_fixed_negative), c.end());
        if(c.empty()) throw UNSATError();
        std::sort(c.begin(), c.end());
        c.erase(std::unique(c.begin(), c.end()), c.end());
        auto taut_pair = find_pair_if(c.begin(), c.end(),
                                      [] (Lit l1, Lit l2) { return l1 == lit::negate(l2); });
        if(taut_pair != c.end()) {
            c.clear();
        }
    }
    ...
    void p_fix_and_eq_rewrite_clauses(const std::vector<Lit>& old_to_new) {
        ...
        m_clauses.erase(std::remove_if(m_clauses.begin(), m_clauses.end(),
                        [] (const SCVec& v) { return v.empty(); }), m_clauses.end());
    }

    void p_apply_fix(Var v, Lit l) {
        m_variable_eliminated[v] = true;
        Lit slit = (lit::negative(l) ? lit::negative_lit(v) : lit::positive_lit(v));
    ...
    void p_apply_remap(Var v, Lit m) {
        ...
        m_reconstruction_stack.emplace_back(std::initializer_list<Lit>{p, mneg});
    }
