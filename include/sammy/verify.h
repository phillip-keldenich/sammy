                       ...
                       Reason* reason = nullptr)
{
    if(solution.size() != clauses.num_vars()) {
        return "Error: Solution length (= " + std::to_string(solution.size()) +
               ") does not match number of variables (= " + std::to_string(clauses.num_vars()) + ")!";
    }
    ...
    auto vio_unary = std::find_if(
        clauses.unary_literals().begin(), clauses.unary_literals().end(),
    ...
    if(vio_unary != clauses.unary_literals().end()) {
        ...
        return "Error: Unary clause (" + 
               std::to_string(lit::externalize(*vio_unary)) +
               ") is violated!";
    }
    auto vio_binary = std::find_if(
        clauses.binary_clauses().begin(), clauses.binary_clauses().end(),
        [&] (auto cl) { return !literal_satisfied(cl.first) && !literal_satisfied(cl.second); }
    );
    ...
    for(CRef c = 1, n = clauses.literal_db_size(); c < n; c = clauses.next_clause(c)) {
...
inline std::optional<std::string> mutually_exclusive_set_has_error(ClauseDB& clauses, Var n_concrete,
                                                                   const std::vector<Vertex>& vertices_)
{
    // normalize vertices and look for invalid ones
    auto normalize = [] (Vertex v) -> Vertex { return {(std::min)(v.first, v.second), (std::max)(v.first, v.second)}; };
    auto is_bad_vertex = [] (Vertex v) { return lit::var(v.first) == lit::var(v.second); };
    auto is_oor_vertex = [&] (Vertex v) { return lit::var(v.second) >= n_concrete; };
    std::vector<Vertex> vertices;
    vertices.reserve(vertices_.size());
    std::transform(vertices_.begin(), vertices_.end(), std::back_inserter(vertices), normalize);
    auto bad_vertex = std::find_if(vertices.begin(), vertices.end(), is_bad_vertex);
    if(bad_vertex != vertices.end()) {
        return "Error: Vertex " + vertex_to_string(*bad_vertex) + " in mutually exclusive set!";
    }
    auto oor_vertex = std::find_if(vertices.begin(), vertices.end(), is_oor_vertex);
    if(oor_vertex != vertices.end()) {
        return "Error: Vertex " + vertex_to_string(*oor_vertex) + " (n_concrete = " +
               std::to_string(n_concrete) + ") in mutually exclusive set!";
    }
    ...
    for(Vertex v : vertices) {
        if(!present_vertices.emplace(v).second) {
            return "Error: Duplicate vertex " + vertex_to_string(v) + " in mutually exclusive set!";
        }
    }
    ...
    for(std::size_t ind_in_vertices = 0, n_vertices = vertices.size(); ind_in_vertices < n_vertices; ++ind_in_vertices) {
        ...
        for(std::size_t j = ind_in_vertices + 1; j < n_vertices; ++j) {
            ...
            if(propagator.is_open(w.first)) {
                if(!propagator.push_level(w.first) || propagator.is_false(w.second)) {
            ...
            if(propagator.is_open(w.second)) {
                ...
                propagator.pop_level();
                if(pushed_w1) propagator.pop_level();
                if(!pres) {
                    continue;
                }
            } else if(pushed_w1) {
    ...
    for(auto [v,w] : explicitly_check_vertices) {
        ...
        try {
            if(gsolver.solve()) {
                return "Error: Vertices " + vertex_to_string(v) + " and " + vertex_to_string(w) + 
...
inline bool verify_solution(const ClauseDB& clauses, const std::vector<bool>& solution) {
