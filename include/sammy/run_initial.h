...
class InitialHeuristicData {
  public:
    InitialHeuristicData(const ClauseDB& simplified, Var n_concrete, EventRecorder* recorder,
                         const RunInitialConfig& config, ThreadGroup<void>* thread_pool) :
    ...
    void first_solution() {
        ...
        recorder->store_event("FIRST_SOLVE_DONE",
        ...
        update_best_lb(0, clq_solver.random_multistart_best_clique(r, all_spawners));
    ...
    void follow_up_iteration(std::size_t iteration) {
        ...
        auto r = config.random_clique_restarts_per_iteration;
        update_best_lb(iteration, clq_solver.random_multistart_best_clique(r / 2, all_spawners));
        update_best_lb(iteration, clq_solver.random_multistart_best_clique(r / 2, new_spawners));
    }
    ...
    void add_spawners(const std::vector<Vertex>& new_spawners) {
        all_spawners.insert(all_spawners.end(), new_spawners.begin(), new_spawners.end());
        std::sort(all_spawners.begin(), all_spawners.end());
        all_spawners.erase(std::unique(all_spawners.begin(), all_spawners.end()), all_spawners.end());
    }

    void update_best_ub(std::size_t iteration, const std::vector<Vertex>& spawners) {
        update_best_ub(iteration, col_solver.all_classes(), spawners);
    }

    void update_best_ub(std::size_t iteration, const std::vector<SharedDBPropagator>& propagators,
                        const std::vector<Vertex>& spawners) 
    {
        if(best_ub.empty() || propagators.size() < best_ub.size()) {
            ...
            recorder->store_event("UB_IMPROVED", OutputObject{
                                      {"num_configs", propagators.size()},
    ...
    void update_best_lb(std::size_t iteration, std::vector<Vertex> clique) {
        if(clique.size() > best_lb.size()) {
            recorder->store_event("LB_IMPROVED", OutputObject{
                                      {"num_interactions", clique.size()},
                                      {"iteration", iteration + 1}
                                  },
    ...
    std::vector<Vertex>
    reconstruct_lb(SimplifyDatastructure& simplify_ds,
                   SimplifiedInstance& simplified_inst)
    {
    ...
    const std::vector<Vertex>& get_bound() const noexcept {
        return best_lb;
    }
    ...
    std::size_t best_ub_value() const noexcept {
        return best_ub.size();
    }

    ColoringHeuristicSolver& primal_solver() noexcept {
        return col_solver;
    }

    ParallelFastCliqueBuilder& dual_solver() noexcept {
        return clq_solver;
    }
    ...
    PairInfeasibilityMap &infeasibility_map() noexcept {
        return inf_map;
    }
    ...
    Var num_concrete() const noexcept {
        return inf_map.get_n_concrete();
    }
...
run_initial_heuristic(const InputData& input, OutputObject& output, EventRecorder& recorder,
                      ...
                      std::function<std::vector<Vertex>()>& get_best_mut_ex_set)
{
    ...
    if(config.simplify) {
        ...
        simplified.emplace(sammy::run_simplifier(*simplifier, simplification_stats));
        ...
        export_simplification(output, *simplifier, *simplified, &simplification_stats);
    }
    ...
    auto should_stop = [&] (std::size_t iteration) -> bool {
        ...
        if(iteration >= config.goal_iterations && 
           current_time >= config.min_time) return true;
        if(initial_data->best_lb_value() >= initial_data->best_ub_value()) return true;
    ...
    for(actual_iterations = 1; !should_stop(actual_iterations); ++actual_iterations) {
        initial_data->follow_up_iteration(actual_iterations);
    }
    export_solution(output, get_best_solution_fn(), 
                    "initial_heuristic", OutputObject{{"iterations", actual_iterations}});
    export_bound(output, get_best_mut_ex_set(), "initial_heuristic", OutputObject{
                    {"random_clique_restarts", config.random_clique_restarts_per_iteration}
                 });
...
inline std::pair<std::size_t, std::size_t>
run_initial_heuristic(const InputData& input, OutputObject& output, EventRecorder& recorder,
                      RunInitialConfig config, ThreadGroup<void>* thread_pool = nullptr) 
{
    ...
    return run_initial_heuristic(input, output, recorder, config, *thread_pool, 
                                 initial_data, simplifier, simplified, get_best_solution,
                                 get_best_mut_ex_set);
}
