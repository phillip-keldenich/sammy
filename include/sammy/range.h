...
static inline
void split_range(RandomAccessIterator begin, RandomAccessIterator end,
                 std::size_t max_partitions, Callable&& callback)
{
    auto count = std::size_t(end - begin);
    if(max_partitions > count) max_partitions = count;
    ...
    for(std::size_t i = 0; i < max_partitions; ++i) {
        ...
        callback(RandomAccessIterator(current), RandomAccessIterator(current_end));
        current = current_end;
    }
}
...
{
  public:
    RandomSkipIterator(RNGType* rng, Iterator iter, Iterator end, double prob) noexcept :
        rng(rng),
        iter(iter),
        end(end),
        prob(prob)
  ...
  private:
    ...
    typename std::iterator_traits<Iterator>::reference dereference() const noexcept 
    {
        return *iter;
    }
