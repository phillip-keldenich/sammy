...
inline std::size_t estimate_bytes_used(const OutputObject& obj) noexcept {
    if(obj.is_string()) {
        return sizeof(obj) + 
               sizeof(nlohmann::json::string_t) +
               obj.get_ref<const nlohmann::json::string_t&>().capacity();
    }
    if(obj.is_binary()) {
        return sizeof(obj) + 
               sizeof(nlohmann::json::binary_t) +
    ...
    if(obj.is_object()) {
        ...
        std::size_t initial = sizeof(obj) + sizeof(nlohmann::json::object_t) +
            (3 * sizeof(void*) + sizeof(nlohmann::json::string_t) + sizeof(obj)) * ref.size();
        return std::accumulate(ref.begin(), ref.end(), initial,
            [] (std::size_t acc, const auto& p) {
                return acc + estimate_bytes_used(p.second) + p.first.capacity();
            });
    }
    return sizeof(obj);
}
...
class EventRecorder {
  public:
    ...
    void store_event_silent(std::string type) {
        m_events.emplace_back(StoredEvent{std::move(type), now(), OutputObject{}});
    }

    void store_event(std::string type) {
        m_events.emplace_back(StoredEvent{std::move(type), now(), OutputObject{}});
        if(print_events) p_print();
    }

    template<typename... PrintArgs>
    void store_event(std::string type, OutputObject data, PrintArgs&&... p_args) {
        if(!data.is_null() && !data.is_object()) {
            throw std::runtime_error("Event data must be an object or null!");
        }
        m_events.emplace_back(StoredEvent{std::move(type), now(), std::move(data)});
        if(print_events) p_print(std::forward<PrintArgs>(p_args)...);
    }
    ...
    const std::vector<StoredEvent>& events() const noexcept {
        return m_events;
    }

    void set_print_events(bool pe) noexcept {
        print_events = pe;
    }

    void merge_events(const EventRecorder& other, const std::string& worker_id) {
        ...
        std::inplace_merge(m_events.begin(), m_events.begin() + old_size, m_events.end(),
    ...
    std::size_t bytes_used() const noexcept {
        ...
        result += (sizeof(StoredEvent) - sizeof(OutputObject)) * m_events.capacity();
        return std::accumulate(
            m_events.begin(), m_events.end(), result,
  ...
  private:
    void p_print_args(std::ostream& outbuffer) {
        outbuffer << " }";
    }

    template<typename PArg1, typename... PArgs>
    void p_print_args(std::ostream& outbuffer, PArg1&& parg1, PArgs&&... pargs)
    {
...
inline void export_simplification(
    OutputObject& output, 
    const SimplifyDatastructure& simplifier,
    ...
    const SimplificationStats* simplification_stats = nullptr)
{
    ...
    std::transform(compressed.new_to_old.begin(), compressed.new_to_old.end(), 
                   std::back_inserter(new_to_old_external),
                   [] (Var o) { return lit::externalize(lit::positive_lit(o)); });
...
inline void export_bound(OutputObject& output,
                         const std::vector<Vertex>& lb,
                         const std::string& tag,
                         const OutputObject& extra_info)
{
    ...
    auto externalize_vertex = [] (Vertex v) { 
        return std::pair<ExternalLit,ExternalLit>{
            lit::externalize(v.first), lit::externalize(v.second)
        };
    };
    std::transform(lb.begin(), lb.end(), 
                   std::back_inserter(external), externalize_vertex);
    nlohmann::json &json = output[tag];
    json["lb"] = extra_info;
...
inline std::string tolower(const std::string& mixed) {
    ...
    std::transform(mixed.begin(), mixed.end(), std::back_inserter(result), [] (char c) {
...
inline void output_data(const OutputObject& output, const std::filesystem::path& path) {
    auto lowext = tolower(path.extension().string());
    if(lowext == ".json") {
