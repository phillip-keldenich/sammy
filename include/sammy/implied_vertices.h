 ...
 * Returns a list of vertices such that covering all of them guarantees a full cover,
 * by removing all vertices that are implied by another vertex.
 */
inline
std::vector<Vertex> eliminate_implied_vertices(const std::vector<Vertex>& vertices,
                                               SharedDBPropagator& propagator) 
{
    if(propagator.get_current_level() > 0)
        throw std::logic_error("eliminate_implied_vertices: decision level > 0");
    DynamicBitset implied(vertices.size(), false);
    std::vector<std::vector<std::pair<Lit, std::size_t>>> partners_of(2 * propagator.db().num_vars());
        ...
        for(Lit l : propagator.get_trail()) {
            ...
            ps.erase(std::remove_if(ps.begin(), ps.end(), 
                     [&] (std::pair<Lit,std::size_t> entry) {
                        if(implied[entry.second]) return true;
                        if(entry.second == i) return false;
    ...
    for(std::size_t i = 0, n = vertices.size(); i < n; ++i) {
        if(!implied[i]) result.push_back(vertices[i]);
    }
...
class ImpliedVertexCache {
  public:
     ...
     */
    bool is_implied(Vertex v) const noexcept {
        return m_implied_by.count(v);
    }
    ...
    Vertex implying_or_self(Vertex v) const noexcept {
        ...
        if(it == m_implied_by.end()) return v;
        return it->second;
    }
    ...
    std::vector<Vertex> get_universe() const {
        if(have_reduced_universe()) return m_reduced_universe;
    ...
    void remove_implied(std::vector<Vertex>& vertices) const {
        if(!have_reduced_universe()) throw std::logic_error("Universe reduction not performed!");
    ...
    /**
     * Remove the implied vertices from the given list (either through the ad-hoc method,
     * or using a cached reduced universe).
     */
    void remove_implied(std::vector<Vertex>& vertices, SharedDBPropagator& propagator) const {
    ...
    /**
     * Remove the implied vertices from the given list (const propagator version).
     */
    void remove_implied(std::vector<Vertex>& vertices, const SharedDBPropagator& propagator) const {
        if(!have_reduced_universe()) {
    ...
    void replace_implied(std::vector<Vertex>& vertices) const {
        if(!have_reduced_universe()) return;
  ...
  private:
    ...
    struct EliminationAlgorithm {
        explicit EliminationAlgorithm(ClauseDB& clauses,
            ...
            that(that),
            propagator(&clauses),
            universe(that->m_pair_inf_map->collect_vertices(that->m_universe_size)),
            implier_of(universe.size(), std::numeric_limits<std::size_t>::max())
        {}
        ...
        inline void compute_impliers_handle_trail_literal(Lit trail_literal, std::size_t pushed_index);
         ...
         */
        void compress_paths() {
            for(std::size_t v : range(universe.size())) compress_path(v);
        }
...
void ImpliedVertexCache::EliminationAlgorithm::compute_literal_partners_of() {
    ...
    auto sort_partners_list = [] (auto& r) { 
        const auto compare_1st = [] (const auto& e1, const auto& e2) { return e1.first < e2.first; };
        std::sort(r.begin(), r.end(), compare_1st);
    };
    std::for_each(partners_of.begin(), partners_of.end(), sort_partners_list);
}

void ImpliedVertexCache::EliminationAlgorithm::mark_and_merge_two_sorted_lists(Lit implier, Lit implied) {
    auto& left_list = partners_of[implier];
    auto& right_list = partners_of[implied];
    if(left_list.empty() || right_list.empty()) return;

    auto left_in = left_list.begin(), left_out = left_list.begin(), left_end = left_list.end();
    auto right_in = right_list.begin(), right_out = right_list.begin(), right_end = right_list.end();
    
    auto left_skip_implied = [&] () {
        while(is_implied(left_in->second)) {
            if(++left_in == left_end) return false;
        }
        return true;
    };
    ...
    auto right_skip_implied = [&] () {
        while(is_implied(right_in->second)) {
            if(++right_in == right_end) return false;
        }
        return true;
    };
    ...
    auto handle_left_remainder = [&] () {
        if(left_in == left_end) return;
        while(advance_left());
    };

    auto handle_right_remainder = [&] () {
        if(right_in == right_end) return;
        while(advance_right());
    };
...
void ImpliedVertexCache::EliminationAlgorithm::compute_impliers_single_literal() {
...
void ImpliedVertexCache::EliminationAlgorithm::
    compute_impliers_handle_trail_literal(Lit trail_literal, std::size_t pushed_index) 
{
    ...
    ps.erase(
        std::remove_if(ps.begin(), ps.end(), 
            [&] (std::pair<Lit,std::size_t> entry) {
                ...
                if(is_implied(entry.second)) return true;
                // avoid detecting vertices as implying themselves
                if(entry.second == pushed_index) return false;
                // if the other literal is true, pushed_index implies entry.first
                if(propagator.is_true(entry.first)) {
...
void ImpliedVertexCache::EliminationAlgorithm::limited_compute_impliers(double time_limit) {
    ...
    for(std::size_t vertex_index : vertex_indices) {
        if(is_implied(vertex_index)) continue;
        ...
        if(++check_count == 16384) {
            check_count = 0;
            if(seconds_between(begin_time, std::chrono::steady_clock::now()) >= time_limit) {
                break;
            }
        }
    }
...
void ImpliedVertexCache::EliminationAlgorithm::compress_path(std::size_t vi) {
    ...
    if(cval == std::numeric_limits<std::size_t>::max() || cval == vi) return;
    ...
    for(;;) {
        ...
        if(nval == std::numeric_limits<std::size_t>::max()) break;
...
void ImpliedVertexCache::limited_reduce_universe(ClauseDB& clause_db, double time_limit) {
