...
class GurobiCliqueSolverG2 {
  public:
    ...
    GurobiCliqueSolverG2(UniverseSubgraph* subgraph, Var n_all,
        ...
        m_nall(n_all),
        m_indset_builder(subgraph->independent_set_builder()),
        ...
        m_best_mes(initial_mut_ex_set)
    {
        ...
        m_model.set(GRB_IntParam_Threads, int(m_subgraph->thread_group().num_threads() + 1));
     ...
     */
    void abort() {
        if(!m_abortable) throw std::logic_error("Trying to abort non-abortable solver!");
     ...
     */
    template<typename ConfigurationIterator> inline 
    void add_configurations(ConfigurationIterator cbeg, ConfigurationIterator cend);

    /**
     * Add a configuration as constraint.
     */
    template<typename ConfigurationType> inline
    void add_configuration(const ConfigurationType& cfg);
    ...
    /**
     * Add the vertices from vertices using add_vertices that are not already in the graph.
     */
    inline void add_new_vertices(const std::vector<Vertex>& vertices) {
        std::vector<Vertex> actual_vertices;
        std::copy_if(vertices.begin(), vertices.end(), std::back_inserter(actual_vertices),
                     [&] (Vertex v) {return !m_subgraph->has_vertex(v);});
        if(!actual_vertices.empty()) add_vertices(actual_vertices);
    }
     ...
     */
    inline
    SolverState solve_relaxation(double time_allowed = std::numeric_limits<double>::infinity());
     ...
     */
    inline
    SolverState solve_full_relaxation(double time_allowed = std::numeric_limits<double>::infinity());

    /**
     * Solve the problem as MIP.
     * Temporarily transforms the problem into a MIP.
     */
    inline SolverState solve_as_mip(double time_allowed = std::numeric_limits<double>::infinity());
     ...
     */
    inline void add_potentially_incomplete_independent_set(const SharedDBPropagator& propagator);

    /**
     * Add an independent set given as list of vertex indices.
     */
    inline void add_direct_independent_set(const std::vector<std::size_t>& indices);
     ...
     */
    void update_best_solution(const std::vector<std::vector<bool>>& new_solution) {
        if(new_solution.size() >= m_best_sample.size()) return;
    ...
    double get_last_value() const noexcept {
        return m_last_objective_value;
    }
    ...
    inline void count_compressible_constraints(std::size_t min_iterations_without_relevance) {
        ...
        for(std::size_t i = 0; i < ncmp; ++i) {
            if(m_complete_independent_set_zero_since[i] >= min_iterations_without_relevance) {
                ++ccmp;
            }
        }
        for(std::size_t i = 0; i < ninc; ++i) {
            if(m_inc_independent_set_zero_since[i] >= min_iterations_without_relevance) {
                ++cinc;
            }
        }
        ...
        m_recorder->store_event("COUNTED_COMPRESSIBLE", {
                                ...
                                }, "incomplete_reducible", "complete_reducible", "percent_reducible");
    }
     ...
     */
    inline void
    export_highest_weights_to_primal(ColoringHeuristicSolver& primal, std::size_t num_constraints);
    ...
    /**
     * Use a vector of complete configurations to try to find violated cutting planes.
     */
    inline std::size_t 
    ...
    /**
     * Use a sample of complete configurations to try to find violated cutting planes.
     */
    inline std::size_t
  ...
  private:
    ...
    // the currently extracted solution (vertex_index, value pairs), sorted by decreasing value
    ...
    void p_foreach_literal(Callable&& callable);

    const std::vector<std::vector<bool>>& p_to_sample(const std::vector<std::vector<bool>>& s) {
        return s;
    }

    std::vector<std::vector<bool>> p_to_sample(const std::vector<DynamicBitset>& s) {
        std::vector<std::vector<bool>> result;
        std::transform(s.begin(), s.end(), std::back_inserter(result), [] (const DynamicBitset& b) {
     ...
     */
    inline void p_extract_ordered_solution(bool possible_suboptimal_mip = false);

    /**
     * Turn a fractional bound on the objective value into an integer one,
     * leaving a small gap for numerical issues.
     */
    inline std::size_t p_make_objective_value_integral(double fractional) const noexcept;

    /**
     * Try to use rounding to turn the current fractional solution into a clique.
     */
     ...
     */
    inline bool p_prohibit_extend_existing(std::size_t v_index, DynamicBitset& covered_vne);
     ...
     */
    inline void p_add_vertices_to_independent_sets(std::vector<Vertex>& vertices,
     ...
     */
    template<typename VertexIterator>
    inline bool p_collect_good_or_possible(VertexIterator vbeg, VertexIterator vend,
     ...
     */
    inline void p_categorize_good_vertices(std::vector<Vertex>& good, 
                                           VertexMapTo<double>& interesting_scores,
    ...
    /**
     * Extract an incomplete independent set (constraint) and add it as initially
     * present configuration to the given primal heuristic solver.
     */
     ...
     */
    inline bool p_direct_is_compatible(const DirectIndependentSet& direct, std::size_t index);
...
template<typename ConfigurationIterator> inline 
void GurobiCliqueSolverG2::add_configurations(
    ConfigurationIterator cbeg, ConfigurationIterator cend)
{
    using ConfigType = typename std::iterator_traits<ConfigurationIterator>::value_type;
    std::for_each(cbeg, cend, [this] (const ConfigType& cfg) {
        add_configuration(cfg);
    });
}

template<typename ConfigurationType> inline
void GurobiCliqueSolverG2::add_configuration(const ConfigurationType& cfg)
{
    ...
    m_complete_independent_set_constraints.push_back(m_model.addConstr(m_expr_buffer <= 1));
}

template<typename Callable> inline
void GurobiCliqueSolverG2::p_foreach_literal(Callable&& callable)
{
...
SolverState GurobiCliqueSolverG2::solve_relaxation(double time_allowed)
{
    ...
    //m_recorder->store_event("DONE_SOLVE_RELAXATION", {{"bound", m_last_objective_value}}, "bound");
    ...
    return is_now_opt ? SolverState::OPTIMUM_ON_SUBGRAPH : 
        (new_clique_size > prev_clique_size ? 
            SolverState::IMPROVEMENT_FOUND : SolverState::NO_IMPROVEMENT_FOUND);
}

void GurobiCliqueSolverG2::p_extract_ordered_solution(bool possible_suboptimal_mip) {
    ...
    std::sort(m_ordered_solution.begin(), m_ordered_solution.end(),
              [](const auto& p1, const auto& p2) { return p1.second > p2.second; });
    if(possible_suboptimal_mip) {
        m_last_objective_value = m_model.get(GRB_DoubleAttr_ObjBound);
        m_subgraph_upper_bound = p_make_objective_value_integral(m_last_objective_value);
    } else {
...
std::size_t GurobiCliqueSolverG2::p_make_objective_value_integral(double v) const noexcept {
...
void GurobiCliqueSolverG2::p_ordered_solution_to_clique() {
    if(m_subgraph_upper_bound > m_best_mes.size()) {
        ...
        if(m_clique_builder.size() > m_best_mes.size()) {
            m_recorder->store_event("IMPROVED_LB", 
...
SolverState GurobiCliqueSolverG2::solve_full_relaxation(double time_allowed) {
    ...
    m_recorder->store_event("BEGIN_SOLVE_FULL_RELAXATION", {
                            { "incomplete_sets", m_inc_independent_sets.size() }, 
                            { "complete_sets", m_complete_independent_sets.size() },
                            { "n", m_subgraph->n() }}, "incomplete_sets", "complete_sets", "n");
    auto before = Clock::now();
    bool any_improvement_found = false;
    for(;;) {
        double time_remaining = time_allowed - seconds_between(before, Clock::now());
        switch(solve_relaxation(time_remaining)) {
          ...
          case SolverState::TIMEOUT_NO_IMPROVEMENT:
            ...
            return any_improvement_found ?
              SolverState::TIMEOUT_IMPROVEMENT : SolverState::TIMEOUT_NO_IMPROVEMENT;
...
bool GurobiCliqueSolverG2::prohibit_violated_nonedges() {
    ...
    for(std::size_t i = 0; i < nve; ++i) {
        ...
        if(vit != last_first && !covered_vne[i]) {
            if(p_prohibit_extend_existing(i, covered_vne)) continue;
            last_first = vit;
...
void GurobiCliqueSolverG2::add_direct_independent_set(const std::vector<std::size_t>& indices) {
    ...
    m_direct_independent_set_constraints.push_back(m_model.addConstr(m_expr_buffer <= 1));
}
    ...
    const SharedDBPropagator& propagator)
{
    if(propagator.get_trail().size() == m_nall) {
        ...
        m_inc_independent_set_constraints.push_back(m_model.addConstr(m_expr_buffer <= 1));
    }
}
...
void GurobiCliqueSolverG2::add_vertices(const std::vector<Vertex>& vertices) {
    ...
    if(old_size + vertices.size() != new_size) {
        throw std::logic_error("Duplicate or already existing vertices passed to add_vertices!");
    }
    for(Vertex v : vertices) {
        GRBColumn column;
        for(std::size_t i = 0, ni = m_complete_independent_sets.size(); i != ni; ++i) {
...
void GurobiCliqueSolverG2::p_categorize_good_vertices(
    std::vector<Vertex>& good, 
    VertexMapTo<double>& interesting_scores,
    std::size_t num_categories, std::size_t goal_vertices) 
{
    if(goal_vertices >= good.size()) return;
    ...
    auto category_of = [&] (double value) -> std::size_t {
        if(value <= 0) return 0; 
        if(value >= 1) return num_categories - 1;
    ...
    if(category_count.front() > goal_vertices) {
        goal_vertices = (std::max)(std::size_t(0.2 * category_count.front()), goal_vertices);
    }
    ...
    auto& rng = sammy::rng();
    good.erase(std::remove_if(good.begin(), good.end(), [&] (Vertex v) {
                   std::size_t cat = category_of(interesting_scores.at(v));
                   if(cat < take_all_from_below) return false;
                   if(cat > take_all_from_below) return true;
                   return bool(std::uniform_real_distribution<double>{0.0, 1.0}(rng) > last_ratio);
               }), good.end());
}

template<typename VertexIterator>
std::size_t GurobiCliqueSolverG2::price_vertices(VertexIterator vbeg, VertexIterator vend)
{
    std::vector<Vertex> good_vertices, possible_vertices;
    VertexMapTo<double> interesting_scores;
    if(!p_collect_good_or_possible(vbeg, vend, 
                                   good_vertices, possible_vertices, 
                                   interesting_scores)) 
    {
        return 0;
    }
    ...
    if(!good_vertices.empty()) {
        ...
        p_categorize_good_vertices(good_vertices, interesting_scores,
                                   20, std::size_t(0.2 * m_subgraph->n()));
        std::size_t total_taken = good_vertices.size();
        p_add_vertices_to_independent_sets(good_vertices, interesting_scores);
        std::size_t added_to_independent_sets = total_taken - good_vertices.size();
        m_recorder->store_event("ADDED_VERTICES", {
                                  ...
                                  {"type", "good"}}, 
                                  "added_to_graph", "added_to_indsets", 
                                  "considered", "total_good");
        add_vertices(good_vertices);
    } else {
        std::size_t analyzed = possible_vertices.size();
        p_add_vertices_to_independent_sets(possible_vertices, interesting_scores);
        m_recorder->store_event("ADDED_VERTICES", {
                                  {"new_count", possible_vertices.size()}, 
                                  {"total_count", analyzed},
                                  {"type", "possible"}}, "new_count", "total_count", "type");
...
void GurobiCliqueSolverG2::p_update_dual_values() {
    m_complete_independent_dual.reset(
        m_model.get(GRB_DoubleAttr_Pi, 
                    m_complete_independent_set_constraints.data(),
                    int(m_complete_independent_set_constraints.size()))
    );
    m_inc_independent_dual.reset(
        m_model.get(GRB_DoubleAttr_Pi,
                    m_inc_independent_set_constraints.data(),
                    int(m_inc_independent_set_constraints.size()))
    );
    m_direct_independent_dual.reset(
        m_model.get(GRB_DoubleAttr_Pi,
                    m_direct_independent_set_constraints.data(),
        ...
        if(current_weight > 1.01) {
            ...
            for(const auto& e : m_ordered_solution) {
                ...
                if(push_vertex(prop, v) >= 0) {
                    ...
                    m_model.chgCoeff(m_inc_independent_set_constraints[i], m_vars[e.first], 1.0);
                }
            }
            if(current_weight > 1.01) {
                ++num_strengthened;
...
std::size_t GurobiCliqueSolverG2::p_greedy_add_to_direct() {
    ...
    for(std::size_t direct_index = 0, dn = m_direct_independent_sets.size(); direct_index < dn; ++direct_index) {
        ...
        for(auto [index, value] : m_ordered_solution) {
            if(direct.vertex_indices.count(index)) continue;
            Vertex v = m_subgraph->vertex(index);
            ...
            m_model.chgCoeff(m_direct_independent_set_constraints[direct_index], m_vars[index], 1.0);
        }
        if(current_weight > 1.01) {
            ++num_strengthened;
        }
...
bool GurobiCliqueSolverG2::p_greedy_generate_direct_independent_sets() {
    ...
    for(std::size_t it = 0; it < 100; ++it) {
        ...
        double weight = m_indset_builder.greedily_extend_weighted(
            initial_part_begin, second_part_end);
        weight += m_indset_builder.greedily_extend_weighted(
            second_part_begin, second_part_end);
        if(weight > 1.01) {
...
bool GurobiCliqueSolverG2::greedy_generate_cutting_planes() {
    ...
    for(OSolIter cbegin = m_ordered_solution.begin(), cend = m_ordered_solution.end(); 
        cbegin != cend; ++cbegin) 
    {
        added_weights = 0.0;
    ...
    if(violated.size() > 10) {
        ...
        std::nth_element(violated.begin(), pos, violated.end(),
            [] (const IEntry& e1, const IEntry& e2) { return e1.second > e2.second; });
    }
    double max_vio = 1.0;
    for(auto i = violated.begin(); i != pos; ++i) {
        if(i->second > max_vio) max_vio = i->second;
        propagator.reset_or_throw();
    ...
    m_recorder->store_event("GREEDY_CUTTING_PLANES", 
                            {{"count", pos - violated.begin()}, 
                             {"max_violation", max_vio - 1.0}},
                            "count", "max_violation");
    m_have_new_constraints = true;
    return true;
}
...
bool GurobiCliqueSolverG2::p_collect_good_or_possible(
    ...
    VertexMapTo<double>& interesting_scores)
{
    assert(m_inc_independent_sets.empty() || m_inc_independent_dual.get() != nullptr);
    assert(m_complete_independent_sets.empty() || m_complete_independent_dual.get() != nullptr);
    struct Context {
        bool have_good = false;
        bool many_good = false;
    };
    const auto n = m_subgraph->n();
    thread_group().parallel_foreach(vbeg, vend, Context{}, [&] (Context& ctx, Vertex v) {
        //if(ctx.many_good) return;
        if(m_subgraph->has_vertex(v)) return;
        double ex_score = p_price_in_existing(v);
        if(ex_score >= 1.01) return;
        if(ex_score >= 0.99 && ctx.have_good) return;
...
void GurobiCliqueSolverG2::p_complete_to_primal(
    ColoringHeuristicSolver& primal, 
    const DynamicBitset& complete_indset)
{
    SharedDBPropagator propagator = m_subgraph->get_propagator();
    propagator.reset_or_throw();
    for(Var v = 0; v < complete_indset.size(); ++v) {
        Lit l = complete_indset[v] ? lit::positive_lit(v) : lit::negative_lit(v);
        if(propagator.is_false(l) || (!propagator.is_true(l) && !propagator.push_level(l))) {
            throw std::logic_error("Failed to push literal from complete configuration!");
        }
    }
...
SolverState GurobiCliqueSolverG2::solve_as_mip(double time_allowed) {
    if(time_allowed <= 0) return SolverState::TIMEOUT_NO_IMPROVEMENT;
    if(std::isfinite(time_allowed)) {
        m_model.set(GRB_DoubleParam_TimeLimit, time_allowed);
    }
    p_begin_mip();
    ...
    if(solcount > 0) {
        ...
        p_ordered_solution_to_clique();
        p_end_mip();
        if(status == GRB_OPTIMAL) return SolverState::OPTIMUM_ON_SUBGRAPH;
...
void GurobiCliqueSolverG2::external_improved_mes(std::vector<Vertex> external_mes) {
    if(external_mes.size() <= m_best_mes.size()) return;
    ...
    const std::vector<std::vector<bool>>& sample)
{
    ...
    auto vertex_value = [&] (const std::vector<bool>& config, Vertex v) {
        return literal_value(config, v.first) && literal_value(config, v.second);
    };
    std::size_t result = 0;
    for(std::size_t i = 0, ncfg = sample.size(); i < ncfg; ++i) {
        ...
        for(const auto& e : m_ordered_solution) {
            Vertex v = m_subgraph->vertex(e.first);
            if(vertex_value(config, v)) total += e.second;
        }
        if(total > 1.01) {
...
bool GurobiCliqueSolverG2::p_direct_is_compatible(const DirectIndependentSet& direct, std::size_t index) {
    return std::all_of(
        direct.vertex_indices.begin(), direct.vertex_indices.end(), 
