...
class VariantSubproblemSolver {
    ...
    using IncSATCadical = FixedMESIncrementalSATImprovementSolver<CadicalSolver>;
    using SatDSaturCadical = FixedMESSatDSaturSolver<CadicalSolver>;
    using Variant = std::variant<FixedSATCadical, FixedSATKissat, 
                                 IncSATCadical, SatDSaturCadical>;
  ...
  public:
    ...
    VariantSubproblemSolver(PortfolioSolver* portfolio,
        ...
        m_strategy(portfolio->lns_select_strategy()),
        m_worker_id(worker_id),
        m_subproblem(std::move(subproblem))
    {
        try {
            if (m_strategy == "satdsatur|cadical") {
                p_emplace_solver<SatDSaturCadical>(
                    portfolio, std::move(prop), recorder);
            } else if(m_strategy == "fixed_sat|cadical") {
                p_emplace_solver<FixedSATCadical>(
                    portfolio, std::move(prop), recorder);
            } else if(m_strategy == "fixed_sat|kissat") {
                p_emplace_solver<FixedSATKissat>(
                    portfolio, std::move(prop), recorder);
            } else if(m_strategy == "inc_sat|cadical") {
                p_emplace_solver<IncSATCadical>(
                    portfolio, std::move(prop), recorder);
            } else {
                throw std::logic_error(
                    "Unknown LNS strategy: " + m_strategy);
            }
        } catch(...) {
    ...
    std::optional<bool> solve() {
        assert(m_wrapped);
        return std::visit([&] (auto& solver) -> std::optional<bool> {
                return solver.solve();
            }, *m_wrapped);
    }
  ...
  private:
    ...
    void p_emplace_solver(PortfolioSolver* portfolio, 
                          SharedDBPropagator&& prop,
                          EventRecorder* recorder)
    {
        m_wrapped.emplace(
            std::in_place_type<Solver>,
            portfolio, std::move(m_subproblem), std::move(prop),
            recorder, m_worker_id
        );
    }
};
