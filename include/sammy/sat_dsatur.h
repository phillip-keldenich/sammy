 ...
 */
template <std::size_t NClassesMax> class StaticOpenClassTracker {
  public:
    explicit StaticOpenClassTracker(std::size_t num_classes)
        : m_open_classes(), m_num_open_classes(num_classes) {
    ...
    std::size_t num_open() const noexcept { return m_num_open_classes; }
...
class DynamicOpenClassTracker {
  public:
    explicit DynamicOpenClassTracker(std::size_t num_classes)
        : m_open_classes(num_classes, true), m_num_open_classes(num_classes) {}
 ...
 */
template <typename IncrementalSatSolver> class FixedMESSatDSaturSolver {
    ...
    template <typename Tracker> struct VertexInfo {
        VertexInfo(std::size_t num_classes) : class_tracker(num_classes) {}
  ...
  public:
    FixedMESSatDSaturSolver(PortfolioSolver* portfolio,
        ...
        : m_portfolio(portfolio), m_subproblem(std::move(subproblem)),
          m_propagator(std::move(prop)), m_recorder(recorder),
          m_worker_id(worker_id), m_n_all(m_propagator.db().num_vars()),
    ...
    bool check_solution_coverage(const std::vector<DynamicBitset>& solution) {
        ...
        for (const DynamicBitset& config : solution) {
            auto new_end = std::remove_if(
                unchecked.begin(), unchecked.end(), [&config](const Vertex& v) {
  ...
  private:
    ...
    void p_init_tiers(VertexInfosType& vertex_info) {
        constexpr std::size_t num_tiers =
    ...
    void p_init_vertices_implying_literal() {
        std::transform(m_subproblem.uncovered_universe.begin(),
                       ...
                       m_subproblem.uncovered_universe.begin(), [](Vertex v) {
    ...
    void p_add_to_class(std::size_t class_index, std::size_t vertex_index,
        ...
        if (pres < 0) {
            ...
            p_added_to_class(class_index, vertex_index, prop, vertex_info,
                             old_trail_length);
        }
    }
    ...
    int p_continue(VertexInfosType& vertex_info) {
        ...
        static_cast<void>(
            &FixedMESSatDSaturSolver::p_check_in_some_class<VertexInfosType>);
        if (!m_low_class_tiers[0].empty()) {
    ...
    std::optional<int> p_continue_ulim_tier(std::size_t tier,
        ...
        auto new_end = std::remove_if(t.begin(), t.end(), [&](std::size_t vi) {
            auto& vinfo = vertex_info[vi];
            if (vinfo.in_some_class) {
                return true;
            }
    ...
    int p_continue_using_sat(VertexInfosType& vertex_info) {
        assert(std::none_of(
            m_low_class_tiers[0].begin(), m_low_class_tiers[0].end(),
    ...
    void p_sat_solution_to_final_solution() {
        ...
        for (std::size_t ci : range(m_num_configs_allowed)) {
            ...
            for (std::size_t li = 0; li < m_n_all; ++li) {
                bool v = std::visit(
                    overloaded{[&](Lit l) -> bool { return model_map[l]; },
    ...
    void p_sat_solution_to_classes(VertexInfosType& vertex_info) {
        ...
        auto bool_value = [&](LitOrVal l) -> bool {
            return std::visit(
                overloaded{[&](const Lit& l) -> bool { return model_map[l]; },
        ...
        // add explicitized
        for (std::size_t exi = 0, n_ex = m_explicitized.size(); exi < n_ex;
             ++exi)
        {
    ...
    void p_reduce_tier(std::vector<std::size_t>& t,
                       VertexInfosType& vertex_info) {
        auto new_end = std::remove_if(t.begin(), t.end(), [&](std::size_t vi) {
            return vertex_info[vi].explicitized;
        });
        t.erase(new_end, t.end());
    }

    static LitOrVal p_negate(LitOrVal v) {
        return std::visit(overloaded{[](bool& b) -> LitOrVal { return !b; },
                                     [](Lit& l) -> LitOrVal { return -l; }},
                          v);
    }
    ...
    void p_sat_add_formula_on(const std::vector<LitOrVal>& config_vars) {
        ...
        for (auto pair : clauses.binary_clauses()) {
            ...
            if (neg1)
                solver_lit1 = -solver_lit1;
            if (neg2)
                solver_lit2 = -solver_lit2;
        ...
        for (std::size_t ci : range(m_num_configs_allowed)) {
            ...
            for (std::size_t v = 0; v < m_n_all; ++v) {
                if (fixed[v]) {
                    ...
                    config_vars.emplace_back(std::in_place_type<Lit>,
                                             m_solver.new_var());
                }
            }
    ...
    int p_continue_complete_all_classes(VertexInfosType& vertex_info) {
        ...
        for (std::size_t class_index : range(m_num_configs_allowed)) {
            ...
            for (SLit l : prop.get_trail()) {
                ...
                std::visit(overloaded{[&](bool b) {
                                          // if lvar is fixed to true and l is
                                          // negated, or vice versa, that is an
                                          // error.
                                          if (b == neg) {
    ...
    bool p_sat_extend_covered_in_class() {
             ...
             ex_index < now_explicitized; ++ex_index)
        {
            ...
            for (std::size_t ci : range(m_num_configs_allowed)) {
                ...
                {
                    covered_in_class.emplace_back(std::in_place_type<bool>,
                                                  false);
                    continue;
                }
                if (fixed1 && fixed2) {
                ...
                if (fixed1) {
                    covered_in_class.emplace_back(std::in_place_type<Lit>,
                                                  *std::get_if<Lit>(&cval2));
                    continue;
                }
                if (fixed2) {
                    covered_in_class.emplace_back(std::in_place_type<Lit>,
                                                  *std::get_if<Lit>(&cval1));
                    continue;
                }
                // if we don't find this satisfied anyways,
                // we need to add a variable; defer creation
                needs_extra_var.push_back(ci);
                covered_in_class.emplace_back(std::in_place_type<bool>, false);
            }
            if (satisfied) {
