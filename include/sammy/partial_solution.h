...
class PartialSolution {
  public:
    ...
    explicit PartialSolution(Var num_vars, const PairInfeasibilityMap* inf_map,
                             ClassIterator classes_begin, ClassIterator classes_end) :
        m_assignments_with_literal(2 * inf_map->get_n_concrete()),
        m_inf_map(inf_map),
        m_num_vars(num_vars),
        m_row_uncolored(2 * inf_map->get_n_concrete(), true)
    {
        using ClassType = std::remove_reference_t<
            std::remove_cv_t<typename std::iterator_traits<ClassIterator>::value_type>>;

        m_assignments.reserve(std::distance(classes_begin, classes_end));
        m_true_concrete_literals.reserve(std::distance(classes_begin, classes_end));
    ...
    void iterate_all_uncovered(Callable&& callable, bool interruptible = false) {
        ...
        for (Lit lmin = 0; lmin < nclit - 2; ++lmin) {
            ...
            for(Index i : with_lit) {
                m_row_uncolored.binary_subtract(m_true_concrete_literals[i], lmin + 1);
            }
    ...
    template<typename Callable/*(Lit lmin, Lit lmax)*/>
    void iterate_all_uniquely_covered(Callable&& callable, bool cover_is_incomplete = false) {
        if(cover_is_incomplete) {
            p_iterate_uniquely_covered_incomplete(std::forward<Callable>(callable));
        } else {
            p_iterate_uniquely_covered_complete(std::forward<Callable>(callable));
        }
    }
    ...
    void iterate_all_uniquely_covered_with_class(Callable&& callable) {
        ...
        for(Lit lmin = 0; lmin < nclit - 2; ++lmin) {
            ...
            for(Index cci : with_lit) {
                ...
                tmp = covered;
                tmp &= ccbits; // tmp = set of previously covered that are by config cci
                multi_covered |= tmp;
                covered |= ccbits;
            }
            multi_covered.flip(); // multi_covered = everything that's not covered multiple times
            multi_covered &= covered; // multi_covered = everything that's not covered multiple times and not uncovered
            for(Index cci : with_lit) {
    ...
    std::vector<DynamicBitset> remove_assignments(const std::vector<Index>& indices) 
    {
        std::vector<DynamicBitset> result;
        result.reserve(indices.size());
        DynamicBitset is_deleted(m_assignments.size(), false);
        for(Index i : indices) is_deleted[i].set();
        ...
        m_assignments.erase(
            std::remove_if(m_assignments.begin(), m_assignments.end(),
                           [] (const DynamicBitset& b) {return b.size() == 0;}), m_assignments.end());
        m_true_concrete_literals.erase(
            std::remove_if(m_true_concrete_literals.begin(), m_true_concrete_literals.end(),
                           [] (const DynamicBitset& b) {return b.size() == 0;}), m_true_concrete_literals.end());
        std::for_each(m_assignments_with_literal.begin(), m_assignments_with_literal.end(),
            [&] (std::vector<Index>& sinds) {
                std::transform(sinds.begin(), sinds.end(), sinds.begin(), [&] (Index x) {return old_to_new[x];});
                sinds.erase(std::remove(sinds.begin(), sinds.end(), NIL), sinds.end());
            });
        return result;
    }
    ...
    const DynamicBitset& get_assignment(std::size_t class_index) const noexcept {
        return m_assignments[class_index];
    }

    const DynamicBitset& get_true_concrete_literals(std::size_t class_index) const noexcept {
    ...
    Index find_covering_class(Lit lmin, Lit lmax) const {
        ...
        for (Index cls_index : list) {
            if(m_true_concrete_literals[cls_index][find_lit]) return cls_index;
        }
    ...
    std::size_t bytes_used() const noexcept {
        ...
        if(!m_true_concrete_literals.empty()) {
            result += m_true_concrete_literals[0].bytes_used() * m_true_concrete_literals.size();
        }
        result += sizeof(std::vector<Index>) * m_assignments_with_literal.capacity();
  ...
  private:
    ...
    void p_add_class_from_prop(Index class_index, const SharedDBPropagator& prop) {
