...
class ORToolsSolver {
  public:
    ...
    ORToolsSolver(
        ...
        std::optional<DynamicBitset> remaining_configuration) :
            ...
            m_infeasibility_map(infeasibility_map),
            m_clauses(&clause_db),
            m_propagator(m_clauses),
            m_best_local_mes(std::move(best_local_mes)),
    ...
    void set_find_single_improvement(bool find_single_improvement) {
        if(m_initialized_objective)
            throw std::logic_error("Called set_find_single_improvement too late!");
    ...
    SolveResult solve() {
        p_construct_model();
        if(m_abort_flag.load()) return SolveResult::ABORTED;
        ...
        switch(response.status()) {
            ...
            case operations_research::sat::CpSolverStatus::INFEASIBLE:
                ...
                if(m_lower_bound > m_initial_global_lower_bound && m_lower_bound_callback) {
                    m_lower_bound_callback(m_lower_bound, m_all_vertices);
                }
                return SolveResult::SOLUTION_WAS_OPTIMAL;
        }
        if(!m_want_single_improvement) {
            std::size_t new_bound = std::size_t(response.best_objective_bound() + 0.01);
  ...
  private:
     ...
     * Index [c][i] checks for coverage of the ith interaction in configuration c.
     */
    ...
    void p_construct_model() {
        ...
        p_report_event("BEGIN_CONSTRUCT_CPSAT_MODEL",
            ...
            "max_num_configurations", "num_interactions", "single_improvement_search",
            "best_lb", "best_global_lb", "best_local_mes_size");
    ...
    void p_extract_partial_solution(const SolverResponse& response) {
        ...
        for(std::size_t copy_index : range(num_copies)) {
            ...
            if(!p_get_value(usage_var, response)) break;
        ...
        m_partial_solution = PartialSolution(Var(num_vars), m_infeasibility_map,
            new_configurations.begin(), new_configurations.end());
    }
    ...
    void p_add_copy_vars(std::size_t copy_index) {
        ...
        for(Var v = 0, nv = m_clauses->num_vars(); v < nv; ++v) {
            ...
            if(all_open || m_propagator.is_open(p)) {
                copy_vars.emplace_back(std::in_place_type_t<BoolVar>{}, m_model.NewBoolVar());
            } else {
    ...
    void p_copy_add_coverage(std::size_t copy_index) {
        ...
        bool definitely_used = std::holds_alternative<bool>(usage_var) && std::get<bool>(usage_var);
        ...
        for(std::size_t vi = 0, n = m_all_vertices.size(); vi < n; ++vi) {
            ...
            if(all_open || (m_propagator.is_open(v.first) && m_propagator.is_open(v.second))) {
                BoolVar vcov = m_model.NewBoolVar();
                coverage_vars.push_back(vcov);
                BoolVar v1 = std::get<BoolVar>(p_value_of(copy_index, v.first));
                BoolVar v2 = std::get<BoolVar>(p_value_of(copy_index, v.second));
                ...
                if(definitely_used || m_remaining_configuration) {
                    ...
                    // a remaining configuration covering none of m_all_vertices,
                    // we can just add the backwards clause ensuring vcov == v1 & v2
            ...
            if(m_propagator.is_false(v.first) || m_propagator.is_false(v.second)) {
            ...
            // one true, one open: if the open variable is made true, v is covered here!
    ...
    void p_copy_add_clauses(std::size_t copy_index) {
        ...
        for(CRef c = 1, n = m_clauses->literal_db_size(); c < n; c = m_clauses->next_clause(c)) {
    ...
    void p_copy_add_clause(std::size_t copy_index, LitIter lbegin, LitIter lend) {
        ...
        if(!is_already_satisfied) {
            if(m_clause_buffer.empty()) {
                throw std::logic_error("Invalid MES vertex or unsatisfiable configuration model!");
            }
    ...
    void p_add_objective() {
        if(m_want_single_improvement) return;
        std::size_t num_copies = m_covering_assignments.size() - 1;
    ...
    bool p_get_value(VarOrVal v, const SolverResponse& response) {
        return std::visit(overloaded{
                [&] (BoolVar& b) -> bool { 
                    return operations_research::sat::SolutionBooleanValue(response, b); 
                },
