...
class PossiblySimplified {
  public:
    PossiblySimplified(const ClauseDB& original, Var n_concrete) :
        m_original(original),
        m_original_concrete(n_concrete)
    {}
    ...
    void simplify(OutputObject& output) {
        ...
        m_simplified.emplace(sammy::run_simplifier(*m_simplifier, m_simplification_stats));
        m_simplification_stats.capture_after(formula(), n_concrete());
        export_simplification(output, *m_simplifier, *m_simplified, &statistics());
    }
    ...
    {
        if(!m_simplified) return s;
        return m_simplifier->reconstruct_sample(*m_simplified, s);
    }

    std::vector<Vertex>
        reconstruct_mes(const std::vector<Vertex>& mes) const
    {
        if(!m_simplified) return mes;
...
class PrimalDualDriver {
  public:
    PrimalDualDriver(EventRecorder& recorder, OutputObject& output,
        ...
        m_output(&output),
        m_thread_pool(),
        m_instance(formula, n_concrete),
        m_inf_map((p_possibly_simplify(), m_instance.n_concrete())),
        m_fast_col_solver(&m_instance.formula(), m_instance.n_concrete(), &m_thread_pool, &m_inf_map),
        m_fast_clq_solver(SharedDBPropagator(&m_instance.formula()), &m_thread_pool),
        m_state(nullptr),
        m_last_primal_ended_at(m_begin_time)
    {
    ...
    void run() {
        ...
        while(m_state) {
            ...
            if(!next) {
                ...
                if(m_state->is_final()) break;
            }
        }
    }

    bool mes_is_optimal() const noexcept {
        if(!m_best_mes || !m_best_sample) return false;
        if(m_best_mes->size() >= m_best_sample->size()) return true;
    ...
    const std::optional<std::vector<std::vector<bool>>>& get_best_sample() const {
    ...
    bool update_best_sample(const std::vector<std::vector<bool>>& sample,
        ...
        if(!m_best_sample || sample.size() < m_best_sample->size()) {
            m_recorder->store_event("IMPROVED_SAMPLE", {{"size", sample.size()},
                                                        {"source", source}}, 
                                    "size", "source");
    ...
    bool update_best_sample(const std::vector<SharedDBPropagator>& propagators,
                            const char* source)
    {
        if(m_best_sample && m_best_sample->size() <= propagators.size()) return false;
        std::vector<std::vector<bool>> s;
        s.reserve(propagators.size());
        std::transform(propagators.begin(), propagators.end(), std::back_inserter(s),
                       [] (const SharedDBPropagator& prop) { return prop.extract_assignment(); });
    ...
    void update_spawners(InputIterator begin, InputIterator end) {
        ...
        if(m_all_spawners_set.size() != old_size) {
            m_all_spawners.assign(m_all_spawners_set.begin(), m_all_spawners_set.end());
        }
    }
    ...
    bool rerun_primal_with_dual_info() {
        ...
        auto& rec = *m_recorder;
        double fraction = std::uniform_real_distribution<double>{0.0, 1.0}(sammy::rng());
        std::size_t max_num_constraints = std::size_t(fraction * (best_mes.size() - 1));
        ...
        lbsolver.export_highest_weights_to_primal(ubsolver, max_num_constraints);
        ...
        vinitial.erase(std::unique(vinitial.begin(), vinitial.end()), vinitial.end());
        ubsolver.color_lazy(vinitial);
        update_spawners(ubsolver.class_spawners());
        update_best_sample(ubsolver.all_classes(), "dual-initialized heuristic");
        std::size_t cuts_extracted = lbsolver.cuts_from_primal(ubsolver.all_classes(), MAX_CUTS_FROM_PRIMAL);
  ...
  private:
    ...
    class DriverState {
      ...
      private:
        ...
        void p_check_time() {
            if(!std::isfinite(driver->m_config.lb_mip_config.total_mip_timeout)) {
                return;
            }
    ...
    class InitialState : public DriverState {
      public:
        ...
        void main() override {
            ...
            driver.update_best_sample(fcol.all_classes(), "initial coloring heuristic");
            ...
            auto r = driver.m_config.initial_heuristic_config.random_clique_restarts_per_iteration;
            auto best_mes = fclq.random_multistart_best_clique(r, driver.m_all_spawners);
      ...
      public:
        ...
        void main() override {
            ...
            rec.store_event("BEGIN_NEXT_HEURISTIC_SAMPLE",
                            {{"iteration", driver.m_initial_iterations}}, "iteration");
            fcol.reset_coloring();
            fcol.color_lazy(*driver.m_best_mes);
            driver.update_best_sample(fcol.all_classes(), "repeat coloring heuristic");
            driver.update_spawners(fcol.class_spawners());
            ...
            rec.store_event("BEGIN_NEXT_HEURISTIC_MES",
                            {{"iteration", driver.m_initial_iterations}}, "iteration");
            auto r = driver.m_config.initial_heuristic_config.random_clique_restarts_per_iteration;
            driver.update_best_mes(fclq.random_multistart_best_clique(r / 2, driver.m_all_spawners),
                                   "repeat clique heuristic (all spawners)");
            driver.update_best_mes(fclq.random_multistart_best_clique(r / 2, fcol.class_spawners()),
        ...
        std::unique_ptr<DriverState> leave() override {
            const double time_needed = seconds_between(driver->m_begin_time, Clock::now());
            auto goal_iter = driver->m_config.initial_heuristic_config.goal_iterations;
            if(time_needed >= driver->m_config.initial_heuristic_config.max_time ||
               (time_needed >= driver->m_config.initial_heuristic_config.min_time && 
    ...
    class InitialSATColoringState : public DriverState {
      public:
        ...
        void main() override {
            ...
            add_and_make_unique(initial_vertices, ubsolver.class_spawners().begin(), ubsolver.class_spawners().end());
            ...
            GurobiCliqueSolverG2 g2_clique_solver{&subgraph, formula.num_vars(), &rec,
                                                  *driver->get_best_sample(), *driver->get_best_mes(),
                                                  driver->m_config.lb_mip_config};
            p_run_satcoloring(subgraph, g2_clique_solver);
        }

      private:
        void p_run_satcoloring(UniverseSubgraph& subgraph, GurobiCliqueSolverG2& clique_solver) {
            ...
            while(best_bound < best_soln) {
                double trem = driver->m_config.sat_coloring_config.initial_sat_coloring_timeout;
                ...
                SATKColoringSolver ksolve{&subgraph, driver->m_recorder, cindices, best_bound};
                ...
                if(!*ksolve_res) {
                    ...
                    if(p_optimize_on_subgraph(clique_solver)) {
                        driver->update_best_mes(clique_solver.get_best_mes(), "LP rounding during initial SAT coloring");
                        const auto& mes = *driver->get_best_mes();
                        p_update_clique_indices(mes, subgraph);
                        if(mes.size() > best_bound) best_bound = mes.size();
                    }
                } else {
                    std::vector<SharedDBPropagator> classes = 
                    extra_constraints.coloring_to_classes(subgraph.get_propagator(), subgraph,
                    ...
                    if(driver->update_best_sample(ubsolver.all_classes(), "extension of initial SAT coloring")) {
        ...
        bool p_optimize_on_subgraph(GurobiCliqueSolverG2& clique_solver) {
            ...
            for(;;) {
                ...
                case SolverState::TIMEOUT_NO_IMPROVEMENT:
                    return clique_size_before < clique_solver.get_best_mes().size();

                default:
                    if(clique_solver.greedy_add_to_cutting_planes()) continue;
                    if(clique_solver.greedy_generate_cutting_planes()) continue;
                    return clique_size_before < clique_solver.get_best_mes().size();
                }
            }
        }

        void p_update_clique_indices(const std::vector<Vertex>& clique, const UniverseSubgraph& subgraph) {
            cindices.clear();
            std::transform(clique.begin(), clique.end(), std::back_inserter(cindices), 
    ...
    class PrimalDualInitializationState : public DriverState {
      public:
        ...
        void main() override {
            ...
            auto& formula = driver->formula();
            rec.store_event("END_INITIAL_SUBGRAPH_CREATION", {{"n", sg.n()}}, "n");
            rec.store_event("BEGIN_INITIAL_MODEL_CREATION");
            driver->m_cnp_clq_solver.emplace(&sg, formula.num_vars(), driver->m_recorder,
      ...
      private:
        std::vector<std::size_t> p_extract_coloring(const std::vector<std::vector<bool>>& sample,
        ...
        void p_run_satcoloring() {
            std::vector<Vertex> verts = driver->m_fast_col_solver.class_spawners();
            verts.insert(verts.end(), driver->m_best_mes->begin(), driver->m_best_mes->end());
            std::sort(verts.begin(), verts.end());
            verts.erase(std::unique(verts.begin(), verts.end()), verts.end());
            UniverseSubgraph subgraph(&driver->formula(), &driver->m_thread_pool,
                                      &driver->m_inf_map, verts);
            subgraph.extend_matrix_by_propagation();
            SATColoringSolver solver{&subgraph, driver->m_recorder,
                                     *driver->m_best_mes, 
                                     p_extract_coloring(*driver->m_best_sample, verts)};
            solver.optimize_coloring();
            auto [classes, spawners] = solver.coloring_to_classes();
            driver->m_recorder->store_event("CLASSES_FROM_COLORING", {{"num_classes", classes.size()}}, "num_classes");
            auto& fcol = driver->m_fast_col_solver;
            fcol.reset_coloring();
            ...
            driver->update_best_sample(fcol.all_classes(), "extended SAT coloring");
            driver->update_spawners(fcol.class_spawners());
            subgraph.add_vertices(fcol.class_spawners());
            SATColoringSolver solver2{&subgraph, driver->m_recorder, 
                                      *driver->m_best_mes,
                                      p_extract_coloring(*driver->m_best_sample, subgraph.vertex_set())};
            ...
            fcol.color_lazy(driver->m_all_spawners);
            driver->m_recorder->store_event("LAZY_COLORING_RESULT", {{"num_classes", fcol.all_classes().size()}}, "num_classes");
        }
        ...
        void p_create_subgraph() {
            ...
            const auto& last_spawners = driver->m_fast_col_solver.class_spawners();
            const auto& all_spawners = driver->m_all_spawners;
            initial.insert(initial.end(), last_spawners.begin(), last_spawners.end());
            std::sort(initial.begin(), initial.end());
            initial.erase(std::unique(initial.begin(), initial.end()), initial.end());
            if(initial.size() < 2000 && all_spawners.size() < 20000) {
                initial.insert(initial.end(), all_spawners.begin(), all_spawners.end());
                std::sort(initial.begin(), initial.end());
                initial.erase(std::unique(initial.begin(), initial.end()), initial.end());
            }
            opt_sg.emplace(&formula, &driver->m_thread_pool, 
                           &driver->m_inf_map, std::move(initial));
        ...
        void main() override {
            ...
            switch(sol_state) {
                ...
                case SolverState::IMPROVEMENT_FOUND:
                    driver.update_best_mes(lbsolver.get_best_mes(),  "LP rounding");
                    successor = std::make_unique<RerunPrimalHeuristicState>(&driver);
                    return;
                ...
                case SolverState::NO_IMPROVEMENT_FOUND:
                    if(driver.p_check_primal_goal_ratio()) {
                        ...
                        successor = std::make_unique<NotOptimalOnSubgraphState>(&driver);
                    }
                    return;
                ...
                case SolverState::OPTIMUM_ON_SUBGRAPH:
                    ...
                    if(driver.update_best_mes(lbsolver.get_best_mes(), "LP rounding") ||
    ...
    class RerunPrimalHeuristicState : public DriverState {
      public:
        ...
        void main() override {
            auto& driver = *this->driver;
            bool primal_added_constraints = driver.rerun_primal_with_dual_info();
    ...
    class NotOptimalOnSubgraphState : public DriverState {
      public:
        ...
        void main() override {
            auto& driver = *this->driver;
            if(driver.m_relaxation_history.size() >= MAX_CUT_ROUNDS_PER_PRICING) {
                p_price_suboptimal();
                driver.m_relaxation_history.clear();
                successor = std::make_unique<PrimalDualBaseState>(&driver);
                return;
      ...
      private:
        void p_price_suboptimal() {
            if(p_price_suboptimal(driver->m_fast_col_solver.class_spawners())) return;
            if(p_price_suboptimal(driver->m_all_spawners)) return;
            p_price_suboptimal(driver->m_fast_col_solver.extract_coloring_order());
        }

        bool p_price_suboptimal(const std::vector<Vertex>& vs) {
            return driver->m_cnp_clq_solver->price_vertices(vs.begin(), vs.end()) > 0;
        }
    };
    ...
    class CutFailedState : public DriverState {
      public:
        ...
        void main() override {
            ...
            if(!driver->m_cnp_clq_solver->price_vertices(valid.begin(), valid.end())) {
    ...
    class CutAndPriceFailedState : public DriverState {
      public:
        ...
        void main() override {
            auto& rec = *driver->m_recorder;
            rec.store_event("CUT_AND_PRICE_FAILED",
    ...
    class OptimalOnSubgraphState : public DriverState {
      public:
        ...
        void main() override {
            ...
            double percent_of_graph = (std::max)(
                MIN_VERTICES_PER_PRICING, MIN_RELATIVE_PER_PRICING * subgraph.n());
            std::size_t pos_vertices = lbsolver.price_vertices(spawners.begin(), spawners.end());
            if(pos_vertices >= percent_of_graph) {
                p_successor_found_new();
                return;
            }
            const auto& all_spawners = driver.m_all_spawners;
            pos_vertices += lbsolver.price_vertices(all_spawners.begin(), all_spawners.end());
            if(pos_vertices >= percent_of_graph) {
                p_successor_found_new();
                return;
            }
            auto coloring_order = ubsolver.extract_coloring_order();
            pos_vertices += lbsolver.price_vertices(coloring_order.begin(), coloring_order.end());
            if(pos_vertices >= percent_of_graph) {
                p_successor_found_new();
                return;
            }
    ...
    class LBOptimalState : public DriverState {
      public:
        ...
        void main() override {
            auto& rec = *driver->m_recorder;
            rec.store_event("OPTIMUM_CLIQUE_ON_G2",
