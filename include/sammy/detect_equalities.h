...
static inline auto implied_literals(const SharedDBPropagator& prop) noexcept {
    return IteratorRange{prop.current_level_begin() + 1, prop.get_trail().end()};
}
...
class ImplicationGraphBuilder {
  public:
    explicit ImplicationGraphBuilder(SimplifyDatastructure* simplify_ds) :
        ...
        clauses(simplify_ds->extract_clause_db()),
        old_counts(clauses.get_clause_counts()),
        propagator(&clauses),
    ...
    bool run_fixing_and_equality_iteration() {
        ...
        do {
            ...
            for(Var v = 0, nv = simplify_ds->original_num_vars(); v < nv; ++v) {
                if(simplify_ds->is_eliminated(v) || !propagator.is_open(lit::positive_lit(v))) {
                    continue;
                }
        ...
        if(changed) {
            ...
            simplify_ds->apply_fixes_and_equalities(equality_graph.compute_old_to_new_map());
        }
        return changed;
    }
  ...
  private:
    ...
    void p_stamp_reachable_black(Lit v, bool clear_first) {
        tj_stack.clear();
        if(clear_first) reachable.clear();
        reachable.insert(v);
    ...
    bool p_add_missing_contrapositivities() {
        ...
        for(Lit v : tj_order) {
            ...
            if(aio.empty()) continue;
            auto& io = implications_of[v];
            p_stamp_reachable_black(v, !std::any_of(io.begin(), io.end(), is_last_pushed));
            rlast = v;
            for(Lit w : aio) {
    ...
    void p_tarjan_scc_strongconnect(Lit v) {
        ...
        for(Lit w : implications_of[v]) { handle_neighbor(w); }
        for(Lit w : additional_implications_of[v]) { handle_neighbor(w); }
    ...
    bool p_tarjan_scc_find_equalities() {
        ...
        for(Lit v = 0; v < nl; ++v) {
            if(tj_dfs_data[v].index != NIL) continue;
    ...
    bool p_push_var(Var v) {
        ...
        if(!p_push_lit(p)) return true;
        p_implications_to_reachable();
        p_analyze_trail(p);
        propagator.pop_level();
        if(!p_push_lit(n)) return true;
        p_analyze_trail(n);
        bool result = p_infer_fixed_from_doubly_implied();
        propagator.pop_level();
        if(result) propagator.incorporate_or_throw();
        return result;
    }
    ...
    void p_analyze_trail(Lit pushed) {
        auto iter_lits = propagator.current_level_begin() + 1, end = propagator.get_trail().end();
        ...
        while(iter_lits < end) {
            ...
            if(rimplied.reason_length == 2) continue;
            p_buffer_clause(rimplied.clause);
            if(clause_analysis_buffer.size() == 2) {
                p_add_double_arc(lit::negate(clause_analysis_buffer[0]), clause_analysis_buffer[1]);
            } else {
    ...
    void p_buffer_clause(CRef clause) {
        ...
        std::copy_if(lits.begin(), lits.end(), std::back_inserter(clause_analysis_buffer),
                     [&] (Lit l) { return propagator.get_decision_level(l) != 0; });
    }
    ...
    struct TarjanSCCDFSInfo {
        ...
        TarjanSCCDFSInfo() noexcept { index = NIL; lowlink = 0; }
    };
                                ...
                                SimplificationStats* stats = nullptr)
{
    ...
    if(count != 0) {
        simplify_ds.apply_fixes_and_equalities(eqgraph.compute_old_to_new_map());
        return true;
    }
    return false;
}
 ...
 */
inline bool detect_failed_and_equal_literals(SimplifyDatastructure& simplify_ds, 
                                             SimplificationStats* stats = nullptr) 
{
    ImplicationGraphBuilder detector{&simplify_ds};
    if(stats) detector.set_stats(stats);
