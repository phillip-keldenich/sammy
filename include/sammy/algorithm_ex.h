...
static inline ForwardIterator find_pair_if(ForwardIterator begin, ForwardIterator end,
                                           Callable&& callable)
{
...
template<typename InputIterator, typename OutputIterator, typename Transformation, typename Predicate>
static inline OutputIterator copy_transformed_if(InputIterator begin, InputIterator end,
                                                 OutputIterator out, Transformation&& transform,
                                                 Predicate&& predicate)
{
...
static inline void add_and_make_unique(Container& container, InputIterator begin, InputIterator end)
{
...
static inline ForwardIterator remove_indices(ForwardIterator begin, ForwardIterator end,
                                             IndexForwardIterator remove_begin, IndexForwardIterator remove_end)
{
    if(remove_begin == remove_end) return end;
    auto first_removed = *remove_begin++;
    ForwardIterator out = begin;
    std::advance(out, first_removed);
    ForwardIterator in = out; ++in;
    auto current_index = first_removed; ++current_index;
    while(remove_begin != remove_end) {
        for(auto next_removed = *remove_begin; current_index < next_removed; ++current_index, ++in, ++out) {
            *out = std::move(*in);
        }
        ++in;
        ++current_index;
...
static inline auto vector(const Range& range) {
    using std::begin;
    using std::end;
...
template<typename IntType>
static inline auto range(IntType end) {
