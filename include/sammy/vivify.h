...
class ClauseVivifier {
  public:
    explicit ClauseVivifier(SimplifyDatastructure* simplify_ds) :
        ...
        clauses((simplify_ds->sort_clauses(), simplify_ds->extract_clause_db())),
        propagator(&clauses),
        first_long_index(p_find_first_long()),
        stats(&stats_buffer)
    {}

    void set_stats(SimplificationStats* stats) {
        this->stats = stats;
    }

    bool run() {
        if(!propagator.get_trail().empty()) return true;
  ...
  private:
    ...
    bool p_vivify_longer(ClauseDB::Lits lits_, CRef coffset) {
        ...
        auto [first_diff_pushed, first_diff_lits] = std::mismatch(pushed.begin(), pushed.end(), lits);
    ...
    void p_vivify_longer_pop_mismatched(std::vector<Lit>::const_iterator first_diff_pushed) {
    ...
    bool p_vivify_ternary(ClauseDB::Lits lits_, CRef coffset) {
        ...
        const Lit* lits = lits_.begin();
        return 
            p_vivify_ternary_pair_conflicts(lits[0], lits[1], lits[2], coffset) ||
            p_vivify_ternary_pair_conflicts(lits[0], lits[2], lits[1], coffset) ||
            p_vivify_ternary_pair_conflicts(lits[1], lits[2], lits[0], coffset);
    }
    ...
    bool p_vivify_ternary_pair_conflicts(Lit l1, Lit l2, Lit l3, CRef coffset) {
        ...
        if(!propagator.is_open(l2)) {
            ...
            // -l1 -> -l2 === l1 v -l2 => resolution with clause on l2 gives l1 v l3
            replace = true;
        ...
        if(replace) {
            ...
            simplify_ds->clauses()[first_long_index + coffset].assign(+replace_clause, replace_clause + 2);
            stats->clauses_strengthened_by_vivification += 1;
        }
        propagator.pop_level();
        if(replace) {
...
inline bool vivify(SimplifyDatastructure& simplifier, SimplificationStats* stats = nullptr) {
    ClauseVivifier vivifier(&simplifier);
    if(stats) vivifier.set_stats(stats);
    return vivifier.run();
}
