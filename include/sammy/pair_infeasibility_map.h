...
static inline void cuda_extract_feasibilities(Var n_concrete,
                                              std::vector<DynamicBitset>& definitely_feasible_matrix,
                                              const std::vector<DynamicBitset>& literals_in_class)
{
    if(literals_in_class.empty()) return;

    const Lit nclit = 2 * n_concrete;
    if(definitely_feasible_matrix.size() != nclit ||
       definitely_feasible_matrix[0].size() != nclit)
    {
        throw std::invalid_argument("definitely_feasible_matrix has wrong size");
    }
    std::size_t u32_per_bitset = literals_in_class[0].blocks().size() * 
                                 sizeof(DynamicBitset::Block) / sizeof(std::uint32_t);
    ...
    detail::CUDADevicePointer<const std::uint32_t> device_bit_data(host_prepare_buffer);
    detail::CUDADevicePointer<std::uint32_t> device_output_buffer(nclit * u32_per_bitset);
    Var v = 0;
    while(v < n_concrete) {
        ...
        detail::call_cuda_extract_kernel(device_bit_data.get(), u32_per_bitset,
                                         device_output_buffer.get(), nclit, v, num_rows,
                                         literals_in_class.size());
        v += num_rows;
    }
    std::vector<std::uint32_t> host_output = device_output_buffer.get_host_copy();
    for(Lit l = 0; l < nclit; ++l) {
        definitely_feasible_matrix[l].binary_or(&host_output[l * u32_per_bitset]);
    }
}
#endif
...
class PairInfeasibilityMap {
  ...
  private:
    ...
    explicit PairInfeasibilityMap(std::size_t n_concrete, const std::vector<std::vector<bool>>& bits) :
        num_vars(n_concrete),
        m_matrix(), m_def_feasible()
    {
  ...
  public:
    ...
    static PairInfeasibilityMap import_bits(const std::vector<std::vector<bool>>& bits) {
        if(bits.size() == 0) throw std::runtime_error("Empty matrix in import_bits!");
        if(bits.size() % 2 == 1) throw std::runtime_error("Odd matrix in import_bits!");
        if(bits[0].size() != bits.size()) throw std::runtime_error("Non-square matrix in import_bits!");
        std::size_t num_concrete = bits.size() / 2;
        return PairInfeasibilityMap{num_concrete, bits};
    }

    void
    incorporate_complete_class(const SharedDBPropagator& propagator) {
        const Lit nc_lit = 2 * num_vars;
        m_incorporate_buffer.clear();
        std::copy_if(propagator.get_trail().begin(), propagator.get_trail().end(),
                     std::back_inserter(m_incorporate_buffer),
                     [&] (Lit l) { return l < nc_lit; });
        std::sort(m_incorporate_buffer.begin(), m_incorporate_buffer.end());
        for(auto i = m_incorporate_buffer.begin(), e = m_incorporate_buffer.end(); i != e; ++i) {
    ...
    void incorporate_complete_classes(const std::vector<DynamicBitset>& literals_in_class, ThreadGroup<void>& tpool) {
        auto handle_var = [&] (Var v) {
            ...
            for(const DynamicBitset& bset : literals_in_class) {
                DynamicBitset& out_row = (bset[plit] ? positive_row : negative_row);
                out_row |= bset;
            }
        };
...
#ifdef SAMMY_CUDA_SUPPORTED
    void cuda_incorporate_complete_classes(const std::vector<DynamicBitset>& literals_in_class, ThreadGroup<void>& tpool) {
        try {
            cuda_extract_feasibilities(num_vars, m_def_feasible, literals_in_class);
			return;
        } catch(const CUDAError& err) {
            std::cerr << "CUDA error in cuda_incorporate_complete_classes: " << err.what() << std::endl;
            had_cuda_error(err);
        }
...
/*
    ...
    void incorporate_complete_classes(PropIterator begin, PropIterator end, ThreadGroup<void>& tpool) {
        std::vector<Lit> prop_trails;
        std::size_t nclasses = std::size_t(end - begin);
        const Var ncvars = num_vars;
        const Lit nclits = 2 * ncvars;
        prop_trails.reserve(nclasses * num_vars);
        std::for_each(begin, end, [&] (const SharedDBPropagator& prop) {
            const auto& t = prop.get_trail();
            std::copy_if(t.begin(), t.end(), std::back_inserter(prop_trails),
        ...
        auto handle_var = [&] (Var v) {
            ...
            for(std::size_t iclass = 0; iclass < nclasses; ++iclass) {
                Row& out = begin[iclass].is_true(p) ? m_def_feasible[p] : m_def_feasible[n];
                auto [tbegin, tend] = trail_range(iclass);
    ...
    collect_vertices(std::size_t reserve_size = 0) const noexcept {
        ...
        for (Lit i = 0, nclit = 2 * num_vars; i < nclit - 2; ++i) {
            std::transform(m_def_feasible[i].ones_from_begin(i + 1),
                           m_def_feasible[i].ones_end(),
                           std::back_inserter(result), [&](std::size_t lmax) {
                               return Vertex{i, Lit(lmax)};
                           });
        }
        return result;
    }
    ...
    std::vector<Vertex>
    sample_vertices(std::size_t target_size, std::size_t total_size = 0) {
        if(!total_size) {
