...
struct InitialPhaseResult {
    ...
    static InitialPhaseResult import_from_output(const OutputObject& obj) {
        auto internalize = [] (const auto& obj, const char* key) {
            return lit::internalize(obj.at(key).template get<std::vector<ExternalVertex>>());
        };
    ...
    OutputObject export_to_output() const {
        return OutputObject{
            ...
            {"best_mutually_exclusive", lit::externalize(best_mutually_exclusive)},
 ...
 */
inline OutputObject 
    export_initial_phase_result(const ClauseDB& clauses, const InitialPhaseResult& result) 
{
    std::stringstream clause_export_stream;
    clauses.export_to(clause_export_stream);
    return OutputObject{
        {"clauses", clause_export_stream.str()},
...
{
    std::stringstream clause_import_stream(obj.at("clauses").get<std::string>());
    return {
