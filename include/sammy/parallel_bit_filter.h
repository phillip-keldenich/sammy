...
class BitsetOperationsBuffer {
  public:
    explicit BitsetOperationsBuffer(ThreadGroup<void>* pool) :
        thread_pool(pool),
        bitsets(pool->num_threads() + 1, Bitset{}),
        used_threads(pool->num_threads() + 1, 0)
    {}

    void prepare() {
        used_threads.assign(thread_pool->num_threads() + 1, 0);
    }

	template<typename Combiner>
    Bitset& initialize(std::size_t index, const Bitset& from_bs, Combiner&& combiner) {
 ...
 * using the thread group of op_buffer, and write the result to initial_and_result.
 */
template<typename BitsetsIterator>
inline void bitwise_filter(BitsetOperationsBuffer& op_buffer, DynamicBitset& initial_and_result,
                           BitsetsIterator begin, BitsetsIterator end)
{
    ...
    if(nt <= 1 || num_sets < 4 * nt || total_size <= 1024 * 1024) {
        std::for_each(begin, end, [&] (const DynamicBitset& bs) { initial_and_result -= bs; });
    } else {
        op_buffer.prepare();
        tgroup.context_function_parallel_foreach_iterator(
            begin, end,
            [&] (std::size_t thread_index, const BitsetsIterator& first_iter) -> Bitset& {
                return op_buffer.initialize(thread_index, *first_iter, [] (Bitset& bout, const Bitset& bin) { bout |= bin; });
            },
        ...
        op_buffer.reduce(initial_and_result, [] (Bitset& in_out, const Bitset& in) { in_out -= in; });
    }
}
 ...
 */
template<typename BitsetsIterator>
inline void bitwise_and(BitsetOperationsBuffer& op_buffer, DynamicBitset& initial_and_result,
                        BitsetsIterator begin, BitsetsIterator end)
{
    auto& tgroup = op_buffer.thread_group();
    auto nt = tgroup.num_threads() + 1;
