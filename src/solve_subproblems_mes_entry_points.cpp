    ...
    for (const auto& entry : mes_history_data) {
        ...
        for (std::size_t i = 0; i < 3; ++i) {
            ...
            run_solver_type(
                ...
                static_cast<FixedMESSATImprovementSolver<CMSAT5Solver>*>(
                    nullptr));
            run_solver_type(
                "fixed_sat[lingeling]",
                static_cast<FixedMESSATImprovementSolver<LingelingSolver>*>(
                    nullptr));
            ...
            run_solver_type(
                ...
                static_cast<
                    FixedMESIncrementalSATImprovementSolver<CMSAT5Solver>*>(
                    nullptr));
            run_solver_type(
                "incremental_sat[lingeling]",
                static_cast<
                    FixedMESIncrementalSATImprovementSolver<LingelingSolver>*>(
                    nullptr));

            // SATDSatur (different interface)
            run_cnpsatdsatur("satdsatur[cadical]",
                             static_cast<CadicalSolver*>(nullptr));
            run_cnpsatdsatur("satdsatur[cryptominisat]",
                             static_cast<CMSAT5Solver*>(nullptr));
            run_cnpsatdsatur("satdsatur[lingeling]",
                             static_cast<LingelingSolver*>(nullptr));
        }
