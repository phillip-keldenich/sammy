...
InitialPhaseResult run_initial_phase(EventRecorder& rec, SimplifiedInstance& instance,
                                     const ExperimentFlagsConfig& config)
{
    ...
    InitialHeuristicData initial{instance.formula, instance.num_concrete, &rec,
                                 config.initial_heuristic_config, &initial_pool};
          ...
          (iteration < iconf.goal_iterations || secs < iconf.min_time)) 
    {
        ...
        if(secs - secs_last > 0.75) {
            rec.store_event("INITIAL_ITERATION_DONE", {{"iteration", iteration}}, "iteration");
            secs_last = secs;
        } else {
            rec.store_event_silent("INITIAL_ITERATION_DONE", {{"iteration", iteration}});
        }
    }
    std::size_t universe_size = initial.infeasibility_map().count_vertices();
    std::vector<Vertex> coloring_order = initial.primal_solver().extract_coloring_order();
    rec.store_event("INITIAL_PHASE_DONE", {{"ub", initial.get_solution().size()},
...
int main(int argc, char** argv) {
    ...
    boost::program_options::options_description extra_options("Barrage options");
    extra_options.add_options()
        ("time-limit", value_with_default(time_limit), "Time limit for the portfolio solver.")
        ("implied-reduction-limit", value_with_default(implied_reduction_limit), 
         "Maximum number of valid interactions to attempt removal of implied interactions for.")
        ("exact-limit", value_with_default(exact_limit),
         "Maximum number of non-eliminated interactions to attempt an exact solve for.")
        ("all-concrete", bool_switch(all_concrete),
         "If set, all features are considered concrete instead of the number given in the instance.")
        ...
        ("old-lns", bool_switch(old_lns), "use the old LNS worker (CliqueSatDSatur)")
        ("max-lns-workers", value_with_default(max_lns_workers), "Maximum number of LNS workers to run.");
    get_config_or_exit(argc, argv, config, extra_options, true, false);

    config.initial_heuristic_config.max_time = std::min(config.initial_heuristic_config.max_time, time_limit);
    config.initial_heuristic_config.min_time = std::min(config.initial_heuristic_config.min_time, time_limit / 3.0);
    ...
    if(all_concrete) input.num_concrete = input.formula.num_vars();
    ...
    InitialPhaseResult initial_result = run_initial_phase(recorder, *simplified, config);
    ...
    if(!dump_initial_phase_file.empty()) {
        OutputObject data_dump = export_initial_phase_result(simplified->formula, initial_result);
    ...
    // setup & run portfolio
    PortfolioSolver solver{clause_db_ticket, &recorder, std::move(initial_result)};
    ...
    {
        have_exact = true;
        setup_element(solver.emplace_element<ExactElement>(&solver, &CliqueSatDSaturExactSolverCore::factory, "Exact Clique & SatDSatur"));
    }
    setup_element(solver.emplace_element<CNPElement>(&solver, &CutAndPricePortfolioCore::factory, "Cut & Price"));
    std::size_t remaining_threads = std::thread::hardware_concurrency() - 1 - std::size_t(have_exact);
    if(remaining_threads > max_lns_workers) {
        remaining_threads = max_lns_workers;
    }
    for(std::size_t i = 0; i < remaining_threads; ++i) {
        if(old_lns) {
            setup_element(solver.emplace_element<OldLNSElement>(&solver, solver.get_best_solution_size(), i));
        } else {
            setup_element(solver.emplace_element<LNSElement>(&solver, &LNSCore::factory, "Non-Incremental SAT LNS"));
        }
    }
    ...
    output["initial_iteration_goal"] = config.initial_heuristic_config.goal_iterations;
    ...
    output["optimal"] = (solver.get_best_lower_bound() == solver.get_best_solution_size());
    auto assignments = solver.get_best_solution().assignments_as<std::vector<bool>>();
    output["best_solution"] = simplifier->reconstruct_sample(*simplified, assignments);
    output["mutually_exclusive_set"] = lit::externalize(simplifier->reconstruct_lb(*simplified, solver.get_best_mes()));
    output["lb_vertices"] = lit::externalize(simplifier->reconstruct_lb(*simplified, solver.get_best_lower_bound_vertex_set()));
    export_events(output, recorder.events());
    if(!config.output_file.empty()) output_data(output, config.output_file);
    return 0;
}
