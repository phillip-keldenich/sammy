...
static std::vector<Vertex> filter_universe_if_covered(const std::vector<Vertex>& universe, const std::vector<DynamicBitset>& literals_in_class) {
	auto is_covered_by = [&] (Vertex v, const DynamicBitset& bset) { return bset[v.first] && bset[v.second]; };
	auto is_uncovered = [&] (Vertex v) {
		return !std::any_of(literals_in_class.begin(), literals_in_class.end(), [&] (const DynamicBitset& bset) { return is_covered_by(v, bset); });
	};
	std::vector<Vertex> remaining;
	std::copy_if(universe.begin(), universe.end(), std::back_inserter(remaining), is_uncovered);
	return remaining;
}

static std::vector<std::vector<Index>> make_classes_with_literal(const std::vector<DynamicBitset>& literals_in_class) {
...
TEST_CASE("[CUDA Iteration] Test with simple instance and partial solutions") {
	std::vector<ExternalClause> external_clauses = {
		{-1, 2}, {3, 4}, {2, -3}
	};
	Var num_vars = 4;
	Var n_concrete = 4;
	...
	auto collect_uncovered = [&] (Lit lmin, Lit lmax) { uncovered_vout.emplace_back(lmin, lmax); };
	...
	for(Lit lmin = 0; lmin < nclit - 2; ++lmin) {
		for(Lit lmax = lmin + 1; lmax < nclit; ++lmax) {
			if(lit::negate(lmin) == lmax) continue;
			if(std::find(excluded.begin(), excluded.end(), std::make_pair(lmin,lmax)) != excluded.end()) continue;
			universe.emplace_back(lmin, lmax);
		}
	}

	SUBCASE("EMPTY PARTIAL") {
		cuda_iterate_all_uncovered(literals_in_class, classes_with_literal, &inf_map, collect_uncovered);
