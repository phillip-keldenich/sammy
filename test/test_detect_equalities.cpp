...
TEST_CASE(
    "[detect_failed_and_equal_literals] Failed and equal literal detection") {
    ClauseDB formula{7, std::vector<ExternalClause>{{1, 2},
                                                    {-1, -2},
                                                    {3, -4},
                                                    {-3, 5},
                                                    {-3, -5, 6},
    ...
    std::vector<bool> full_sol =
        simplify_ds.reconstruct_solution(instance, simp_sol);
...
TEST_CASE("[detect_failed_and_equal_literals] Level 0 literals & subsumption, "
          "soletta") {
    ...
    std::unordered_set<Lit> reduced_original_clause{
        lit::internalize(-296), lit::internalize(-267), lit::internalize(-49),
        lit::internalize(71), lit::internalize(106)};
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        auto sol_reconstructed =
            simplify_ds.reconstruct_solution(simplified, sol_simplified1);
        CHECK(check_error(
            solution_has_error(simplified.formula, sol_simplified1)));
        CHECK(check_error(solution_has_error(clause_db, sol_reconstructed)));
    }
}

TEST_CASE(
    "[detect_failed_and_equal_literals] Failed and equal literals, soletta") {
    std::vector<ExternalClause> clauses = soletta_2017_03_01_15_25_44_clauses();
    Var n_all = 461, n_concrete = 460;
    ClauseDB clause_db{n_all, clauses};
    SimplifyDatastructure simplify_ds{clause_db, n_concrete};
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        auto sol_reconstructed =
            simplify_ds.reconstruct_solution(simplified1, sol_simplified1);
        CHECK(solution_has_error(simplified1.formula, sol_simplified1) ==
              std::nullopt);
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        auto sol_reconstructed2 =
            simplify_ds.reconstruct_solution(simplified2, sol_simplified2);
        CHECK(solution_has_error(simplified2.formula, sol_simplified2) ==
              std::nullopt);
        CHECK(solution_has_error(clause_db, sol_reconstructed2) ==
              std::nullopt);
    }
}
