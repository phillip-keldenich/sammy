...
TEST_CASE("[SATDsatur] Test with known universe") {
    ...
    std::transform(clique.begin(), clique.end(), std::back_inserter(clique_inds),
                   [&] (Vertex v) {return subgraph.vertex_index(v); });
    SATDSaturCoverer<LingelingSolver> coverer{&subgraph, std::move(clique_inds), 10};
...
TEST_CASE("[SATDSatur] Test with soletta instance") {
    ...
    ThreadGroup<void> threads{0};
    ColoringHeuristicSolver solver{&clauses, varcount.n_concrete, &threads, &inf_map};
    solver.set_quiet(true);
    solver.initialize_feasibilities();
    solver.color_lazy();
    solver.extract_feasibilities();
    ...
    std::vector<std::pair<int,int>> known_mes = {
        {74, 131}, {131, 402}, {74, 143}, {143, 402}, 
        {2, 436}, {74, 436}, {2, 131}, {2, 143}, 
        {402, 436}, {279, 402}, {31, 436}, 
        {303, 350}, {303, -359}, {74, 350}, 
        {2, -359}, {74, 351}, {2, 279}, 
        {2, 351}, {31, 351}, {-359, 402}, 
        {131, 303}, {84, -389}, {-70, 381}, 
        {-79, 345}, {-378, 451}, {-142, 370}, {279, 303}
    };
    std::vector<Vertex> best_mes;
    std::transform(known_mes.begin(), known_mes.end(), std::back_inserter(best_mes),
                   [] (std::pair<int,int> ve) {
                       return Vertex{lit::internalize(ve.first), lit::internalize(ve.second)};
                    });
    ...
    CHECK(!mutually_exclusive_set_has_error(clauses, varcount.n_concrete, best_mes));
    std::vector<Vertex> uniquely_covered;
    best_solution.iterate_all_uniquely_covered([&](Lit lmin, Lit lmax) {
        uniquely_covered.emplace_back(lmin, lmax);
    });
    uniquely_covered.insert(uniquely_covered.end(), best_mes.begin(), best_mes.end());
    std::sort(uniquely_covered.begin(), uniquely_covered.end());
    uniquely_covered.erase(std::unique(uniquely_covered.begin(), uniquely_covered.end()), uniquely_covered.end());
    UniverseSubgraph subgraph{&clauses, &threads, &inf_map, uniquely_covered};
    SATDSaturCoverer<LingelingSolver> coverer{&subgraph, subgraph.to_indices(best_mes), best_solution.size()};
    coverer.run_coloring(false);
    CHECK(coverer.was_successful());
    CHECK(coverer.did_improve());
    std::vector<SharedDBPropagator> incomplete = coverer.get_incomplete_solution();
    solver.reset_coloring();
    ...
    for(std::size_t i : range(solver.all_classes().size())) {
        CHECK(!solution_has_error(clauses, solver.get_partial_solution().get_assignment(i)));
    }
}
