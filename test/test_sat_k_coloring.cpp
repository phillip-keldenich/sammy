...
TEST_CASE("[SATKColoring] SAT coloring validity") {
    ...
    std::vector<std::size_t> clique{
        subgraph.vertex_index(
            Vertex{lit::positive_lit(0), lit::positive_lit(1)}),
        subgraph.vertex_index(
            Vertex{lit::negative_lit(0), lit::positive_lit(1)}),
        subgraph.vertex_index(
            Vertex{lit::positive_lit(0), lit::negative_lit(1)}),
        subgraph.vertex_index(
            Vertex{lit::negative_lit(0), lit::negative_lit(1)})};
    ...
    CHECK(*std::max_element(coloring.begin(), coloring.end()) ==
          coloring_number - 1);
