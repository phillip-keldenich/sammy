...
TEST_CASE("[CliqueSATDSatur] soletta optimal solve (initialized with non-optimal clique)") {
    ...
    ColoringHeuristicSolver solver{&simplified.formula, simplified.num_concrete, &threads, &inf_map};
    ...
    ParallelFastCliqueBuilder clq_builder{SharedDBPropagator(&simplified.formula), &threads};
    ...
    CHECK(!mutually_exclusive_set_has_error(simplified.formula, simplified.num_concrete, best_clique));
    ...
    std::vector<Vertex> filtered_universe = eliminate_implied_vertices(unfiltered_universe, SharedDBPropagator{&simplified.formula});
    filtered_universe.insert(filtered_universe.end(), best_clique.begin(), best_clique.end());
    std::sort(filtered_universe.begin(), filtered_universe.end());
    filtered_universe.erase(std::unique(filtered_universe.begin(), filtered_universe.end()), filtered_universe.end());
    CliqueSatDSaturSolver<LingelingSolver> csds{filtered_universe, &inf_map, simplified.formula,
                                                best_clique, best_clique.size(), best_clique.size(),
