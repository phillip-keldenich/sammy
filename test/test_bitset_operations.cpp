...
static std::size_t generate_random_size(std::size_t min_size,
                                        std::size_t max_size) {
    auto& rng = sammy::rng();
    return std::uniform_int_distribution<std::size_t>(min_size, max_size)(rng);
}

static std::pair<DynamicBitset, std::vector<bool>>
generate_random_set(std::size_t s, double p) {
...
TEST_CASE("[DynamicBitset] Offset bulk operations") {
    ...
    SUBCASE("binary_subtract76") {
        ...
        CHECK(
            same_set(tmp, std::vector<std::size_t>{0, 1, 65, 79, 1025, 12233}));
        tmp = s2;
        tmp.binary_subtract(s1, 76);
        CHECK(
            same_set(tmp, std::vector<std::size_t>{0, 1, 65, 78, 1026, 12231}));
    }
    ...
    SUBCASE("binary_subtract_uint32_t") {
        ...
        CHECK(same_set(tmp, std::vector<std::size_t>{1, 76, 79, 1022, 1025,
                                                     12232, 12233}));
        tmp = s2;
        tmp.binary_subtract(buffer);
        CHECK(same_set(tmp,
                       std::vector<std::size_t>{1, 76, 78, 1022, 1026, 12232}));
    }
}
...
TEST_CASE("[DynamicBitset] Unparallelized bitset bulk operations") {
    for (int i = 0; i < 100; ++i) {
        ...
        c1 &= t1;
        std::transform(c2.begin(), c2.end(), t2.begin(), c2.begin(),
            ...
            if (initial_and_result[i]) {
                CHECK(
                    !std::any_of(sets.begin(), sets.end(),
                                 [i](const Bitset& b) { return bool(b[i]); }));
            } else {
                CHECK(std::any_of(sets.begin(), sets.end(),
                                  [i](const Bitset& b) { return bool(b[i]); }));
            }
        }
    }
