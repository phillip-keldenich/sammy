...
TEST_CASE("[eliminate_implied_vertices] Test implied vertices (soletta unsimplified)") {
    VarCount vc = soletta_2017_03_01_15_25_44_variables();
    std::vector<ExternalClause> raw_clauses = soletta_2017_03_01_15_25_44_clauses();
    ...
    std::vector<Vertex> filtered = eliminate_implied_vertices(universe, SharedDBPropagator{&clauses});
...
TEST_CASE("[eliminate_implied_vertices] Test implied vertices (soletta simplified)") {
    VarCount vc = soletta_2017_03_01_15_25_44_variables();
    std::vector<ExternalClause> raw_clauses = soletta_2017_03_01_15_25_44_clauses();
    ...
    std::vector<Vertex> universe = inf_map.collect_vertices();
    std::vector<Vertex> filtered = eliminate_implied_vertices(universe, SharedDBPropagator{&simplified.formula});
    CHECK(filtered.size() < universe.size());
    solver.reset_coloring();
    solver.add_vertices_to_queue(filtered.begin(), filtered.end());
    solver.color_vertices_in_queue();
