...
TEST_CASE("[PartialSolution] Test with known universe") {
    ...
    std::vector<std::vector<int>> fixed_solution{{-1, -2, 3}, {1, 2, -3},
                                                 {-1, 2, 3},  {1, -2, -3},
                                                 {-1, 2, -3}, {1, -2, 3}};
    ...
    solution.iterate_all_uniquely_covered([&](Lit lmin, Lit lmax) {
        uniquely_covered_pairs.emplace_back(lit::externalize(lmin),
                                            lit::externalize(lmax));
    });
    std::vector<std::pair<int, int>> uniques{{1, 2},   {1, 3}, {-1, -2},
                                             {-1, -3}, {2, 3}, {-2, -3}};
...
TEST_CASE("[PartialSolution] Test with soletta instance") {
    ...
    ColoringHeuristicSolver solver{&clauses, soletta_vars.n_concrete, &threads,
                                   &inf_map};
    solver.set_quiet(true);
    solver.initialize_feasibilities();
    solver.color_lazy();
    ...
    for (Vertex v : uniquely_covered) {
        unique_count_per_class[solver.find_covering_class(v.first, v.second)] +=
            1;
    }
    ...
    for (Vertex v : uniquely_covered) {
        unique_count_per_class[solution.find_covering_class(v.first,
                                                            v.second)] += 1;
    }
    auto removed2 = solution.remove_assignments({0});
    CHECK(removed2.size() == 1);
    CHECK(solution.size() == s - 2);
    CHECK(unique_count_per_class[0] + prev_uncovered ==
          solution.count_uncovered());
    unique_count_per_class.assign(solution.size(), 0);
