...
IncrementalSolver:
    ...
    Lit new_var(bool reusable = true) -> add a new variable. if reusable=false, cannot be used in new clauses/assumptions
                                  after the next solve is run.
    ...
    std::optional<bool> solve(const std::vector<Lit>& assumptions = {}, time_limit=infinity)
    void terminate()
    void reset_terminate()
    void fix(Lit l) // fix a currently-reusable variable to make the given literal true;
...
class LingelingSolver {
  public:
    ...
    void add_short_clause(Args&&... args) {
